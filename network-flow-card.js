...
const DEFAULT_CONFIG = {
  ...
  switch_devices_circle_size: 48,
  switch_devices_icon_size: 18,
  internet: {
    name: "",
    entity: "",
    icon: "mdi:web",
...
// Reshapes a node-level Switch into the same AP-shaped object the
// existing branch-rendering pipeline already expects, so a switch-only
// node can flow through all the same (well-tested) rendering logic as
// a regular AP - single connecting line (switches never have
// bandwidth entities, so the dual-line path never triggers), no
// backhaul badge, not eligible as Primary. This avoids writing a
// second, parallel set of switch-specific render branches.
function switchToApShape(sw) {
  const c = sw.colors || {};
  return {
    _isSwitchNode: true,
    entity: sw.entity,
    name: sw.name,
    icon: sw.icon || "mdi:switch",
    devices_icon: sw.devices_icon || "mdi:devices",
    is_primary: false,
    show_backhaul_icon: false,
    show_primary_badge: false,
    connects_to_switch: false,
    entities: {
      connected_devices: sw.entities?.connected_devices || "",
      download: "",
      upload: "",
      backhaul_type: "",
      backhaul_speed: "",
    ...
    colors: {
      ...
      upload: c.bus_line,
      line: c.bus_line,
      devices_circle: c.devices_circle,
      devices_icon: c.devices_icon,
      devices_line: c.bus_line,
      backhaul_icon: "transparent",
      offline_circle: c.offline_circle,
      offline_icon: c.offline_icon,
      devices_offline_circle: c.devices_offline_circle,
      devices_offline_icon: c.devices_offline_icon
    }
  };
}
...
class NetworkFlowCard extends LitElement {
  ...
  render() {
    ...
    // Nodes are the new schema. A node is AP-only (behaves exactly as
    // before), Switch-only (adapted into AP shape so it flows through
    // the same rendering pipeline), or Switch-with-APs (a "group" -
    // its APs stay in the flat list for rows 4-7 rendering, unchanged,
    // but row 3 for those specific APs gets replaced by a group header
    // spanning all of them: Switch circle + its own mini bus-line).
    const switchGroups = [];
    const flatApsFromNodes = (config.nodes || []).flatMap((node) => {
      if (node.switch && node.access_points && node.access_points.length > 0) {
        const aps = node.access_points;
        switchGroups.push({ switch: node.switch, apRefs: aps });
        return aps;
      }
      if (node.switch) {
        return [switchToApShape(node.switch)];
      }
      return node.access_points || [];
    });
    const apData = flatApsFromNodes.map((ap) => {
    ...
    const primaryIsInSwitchGroup = !!rawPrimaryApItem && switchGroups.some(
      (g) => g.apRefs.includes(rawPrimaryApItem.ap)
    );
                  ...
                  ? (() => {
                      ...
                      return html`
                        <div class="lan-branch ${summaryPos === "right" ? "flip-left" : ""}">
                          ...
                          <div
                            class="circle-wrap"
                            style="width:${this._config.switch_devices_circle_size ?? 48}px; height:${this._config.switch_devices_circle_size ?? 48}px; opacity:${swDevOffline ? 0.6 : 1};"
                            @click=${(e) => {
                              e.stopPropagation();
                          ...
                          >
                            <div class="circle ${swDevOffline ? "circle-pulse" : ""}" style="border-color:${swDevCircleColor}; --pulse-color:${swDevCircleColor};">
                              <ha-icon class="${swDevOffline ? "icon-pulse" : ""}" .icon=${swDevOffline ? "mdi:exclamation-thick" : (switchConfig.devices_icon || "mdi:devices")} style="color:${swDevIconColor};--mdc-icon-size:${this._config.switch_devices_icon_size ?? 18}px"></ha-icon>
  ...
  _renderBranches(apData, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, switchGroups = []) {
    ...
    // Resolve each Switch group's AP references to their actual index
    // within this specific apData array (not the outer, unfiltered
    // one), then to grid columns, so the group header can span exactly
    // the right range regardless of how the Primary AP filter may have
    // shifted positions.
    const resolvedGroups = switchGroups
      .map((g) => {
        const idxs = g.apRefs
          .map((apRef) => apData.findIndex((item) => item.ap === apRef))
          .filter((i) => i !== -1);
        if (!idxs.length) return null;
        const cols = idxs.map(apCol);
        return { switch: g.switch, minCol: Math.min(...cols), maxCol: Math.max(...cols), apIdxs: idxs };
      })
      .filter(Boolean);
    const apIdxToGroup = new Map();
    resolvedGroups.forEach((g) => g.apIdxs.forEach((i) => apIdxToGroup.set(i, g)));
    ...
    return html`
      <div
        ...
        style="grid-template-columns:repeat(${totalCols}, max-content); grid-template-rows:${collapseTbar ? '0px' : (routerEnabled ? '26px' : '0px')} ${collapseTbar ? '0px' : '2px'} auto auto auto auto 24px; column-gap:${this._config.ap_column_gap ?? 32}px; transform:translateX(${fallbackShift}px);"
      >
        ${collapseTbar
          ? null
          : !routerEnabled
          ...
          (item, i) => {
            ...
            <div class="ap-col-line ${inGroup ? "ap-col-line-grouped" : ""}" style="grid-column:${apCol(i)}">
              ${hideApOwnLine
                ...
                : inGroup
                ? this._verticalSingleLine(
                    apIdxToGroup.get(i).switch.colors?.bus_line || "var(--divider-color, #ccc)",
                    3,
                    animate,
                    28,
                    false
              ...
              ${!hideApOwnLine && !apLineOffline && !inGroup
        ...
        ${resolvedGroups.map((group) => {
          const swOffline = isEntityUnavailable(hass, group.switch.entity);
          const swColor = swOffline
            ? group.switch.colors?.offline_circle || "var(--error-color)"
            : group.switch.colors?.circle;
          const swIconColor = swOffline
            ? group.switch.colors?.offline_icon || "var(--error-color)"
            : group.switch.colors?.icon;
          const swSize = this._config.switch?.circle_size ?? 60;
          const swIconSize = this._config.switch?.icon_size ?? 22;
          const swLabel = getCircleLabel(group.switch.name, getEntityState(hass, group.switch.entity), "Switch");
          const colSpan = group.maxCol - group.minCol + 1;
          return html`
            <div
              class="switch-group-header"
              style="grid-column:${group.minCol} / span ${colSpan}; grid-row:3;"
            >
              <div
                class="circle-wrap"
                style="width:${swSize}px; height:${swSize}px; opacity:${swOffline ? 0.6 : 1};"
                @click=${() => this._handleMoreInfo(group.switch.entity)}
              >
                <div class="circle ${swOffline ? "circle-pulse" : ""}" style="border-color:${swColor}; --pulse-color:${swColor};">
                  <ha-icon class="${swOffline ? "icon-pulse" : ""}" .icon=${swOffline ? "mdi:exclamation-thick" : (group.switch.icon || "mdi:switch")} style="color:${swIconColor};--mdc-icon-size:${swIconSize}px"></ha-icon>
                  <span class="circle-value">${swLabel}</span>
                </div>
              </div>
              ${swOffline
                ? this._renderXOnlyHorizontal(colSpan > 1 ? 60 : 28)
                : html`<div class="switch-group-busline" style="background:${group.switch.colors?.bus_line || 'var(--divider-color, #ccc)'}"></div>`}
            </div>
          `;
        })}
        ${apData.map(
          (item, idx) => {
            const apLabel = getCircleLabel(item.ap.name, item.apEntityState, item.ap._isSwitchNode ? "Switch" : "AP");
            const apSize = item.ap._isSwitchNode
            ...
            return html`
              ...
              >
                <div class="circle ${apOffline ? "circle-pulse" : ""}" style="border-color:${apCircleColor}; --pulse-color:${apCircleColor};">
                  <ha-icon class="${apOffline ? "icon-pulse" : ""}" .icon=${apOffline ? "mdi:exclamation-thick" : (item.ap.icon || "mdi:wifi")} style="color:${apIconColor};--mdc-icon-size:${item.ap._isSwitchNode ? (this._config.switch?.icon_size ?? 22) : (this._config.ap_icon_size ?? 24)}px"></ha-icon>
                  <span class="circle-value">${apLabel}</span>
                </div>
                ${item.ap.is_primary && item.ap.show_primary_badge !== false
                  ? html`<div
                      class="primary-ap-badge"
                      style="background:${item.ap.colors.primary_badge ?? 'var(--orange-color)'}; color:${item.ap.colors.primary_badge_icon ?? 'var(--card-background-color)'}; border:2px solid ${item.ap.colors.primary_badge_border ?? 'transparent'}; width:${this._config.badge_size ?? 18}px; height:${this._config.badge_size ?? 18}px;"
                    >
                      <ha-icon icon="${this._config.primary_badge_icon || 'mdi:star'}" style="--mdc-icon-size:${this._config.badge_icon_size ?? 12}px"></ha-icon>
                    </div>`
                  : null}
            ...
            <div class="ap-col-devline" style="grid-column:${apCol(i)}; ${devLineOffline ? 'display:flex; justify-content:center; align-items:center;' : ''}">
              ${devLineOffline && (item.hasDevices || hasIndividualDevices)
                ? html`
                  ...
                  `
                : !devLineOffline && (item.hasDevices || hasIndividualDevices)
                ? item.ap._isSwitchNode
                  ? html`<div
                      class="ap-col-fillline"
                      style="background:${item.ap.colors.devices_line}"
                    ></div>`
                  : html`<div
                      class="ap-col-fillline"
        ...
        ${apData.map((item, idx) => {
          const devCircleSize = item.ap._isSwitchNode
            ? (this._config.switch_devices_circle_size ?? 48)
            : (this._config.ap_devices_circle_size ?? 56);
          const apLineOffline = routerOffline || isEntityUnavailable(hass, item.ap.entity);
                ...
                >
                  ...
                  >
                    <ha-icon
                      ...
                      style="color:${devIconColor};--mdc-icon-size:${item.ap._isSwitchNode ? (this._config.switch_devices_icon_size ?? 18) : (this._config.ap_devices_icon_size ?? 20)}px"
                    ></ha-icon>
                    <span class="circle-value">
    ...
    return html`
      <div class="form-section">
        ${nodes.map((node, idx) => {
          ...
          const label = node.name || (isSwitchNode ? (node.switch.name || node.switch.entity) : (ap.name || ap.entity)) || `Node ${idx + 1}`;
          return html`
            <div class="list-item">
              <div class="list-item-info">
                <ha-icon .icon=${displayIcon} style="color:var(--primary-color);"></ha-icon>
                <span>${label}</span>
              </div>
    ...
    return html`
      ...
      <div class="two-col">
        <div>
          ...
          ${this._renderColorInput("Connection Line Color", c.bus_line, `${prefix}.colors.bus_line`)}
        </div>
        <div>
  ...
  _renderAdvancedPage() {
    ...
    return html`
      <div class="form-section">
        ...
        ${this._renderSlider("Switch Connected Devices Circle Size", this._config.switch_devices_circle_size, "switch_devices_circle_size", 24, 80)}
        ${this._renderSlider("Switch Connected Devices Icon Size", this._config.switch_devices_icon_size, "switch_devices_icon_size", 8, 40)}
