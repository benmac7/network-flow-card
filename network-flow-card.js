...
function buildNetworkTopology(config) {
  ...
  // it to reach: the Clients box, or a Connected Devices circle of its own.
...
// first.
const SPEEDTEST_PLATFORMS = ["speedtestdotnet", "ookla_speedtest", "cloudflare_speed_test", "librespeed", "fastdotcom"];
...
class NetworkFlowCard extends i {
  ...
  // which drew both strands the moment either one was configured.
  _bandwidthLine(dlColor, ulColor, dlDur, ulDur, hasDl, hasUl, animate, height = 32, dashed = false) {
    if (hasDl && hasUl) return this._verticalDualLine(dlColor, ulColor, dlDur, ulDur, animate, height, dashed);
    if (hasDl) return this._verticalSingleLine(dlColor, dlDur, animate, height, dashed);
    if (hasUl) return this._verticalSingleLine(ulColor, ulDur, animate, height, dashed);
    return null;
  }
    ...
    const apCircle = (t, i, col, cell) => {
          ...
          return b`
            <div
              ...
              @click=${() => this._handleMoreInfo(ap.entity)}
            >
      ...
      >
        ...
        ${!hasAnySwitch ? null : columns.map((col, i) => {
          if (!col.switch) {
            ...
            return b`
              <div class="ap-col-line bus-top-connector" style="grid-column:${topGridCol(col, i)}; grid-row:3;">
        ...
        ${!hasAnySwitch ? null : columns.map((col, i) => {
          ...
          return routerOffline ? null : b`
            <div class="ap-col-line" style="grid-column:${topGridCol(col, i)}; grid-row:4;">
              ${topAp.hasBandwidth
                ? this._bandwidthLine(busLineColor, busLineColor, topAp.dlDur, topAp.ulDur, topAp.hasDl, topAp.hasUl, animate, "100%", !topAp.wired)
                : this._verticalPassThroughLine(busLineColor, topAp.dur, animate, !topAp.wired)}
            </div>
...
class NetworkFlowCardEditor extends i {
  ...
  _renderAccessPointsPage() {
    ...
    return b`
      <div class="form-section">
        ...
        ${this._infoHeader(
          "Auto-Discovery",
          "Scans TP-Link Deco (satellite units, added as Access Point nodes), Proxmox VE (physical/cluster nodes, added as Server nodes with their VMs/Containers attached automatically), AsusRouter AiMesh (satellite units, added as Access Point nodes), and UniFi/Omada (their switches and access points) in one pass. Nothing is added automatically; filter and select which ones you want below, and choose whether each candidate becomes its own new Node or attaches under any Switch or Access Point you've already configured, at any depth."
        )}
              ...
              <div class="tree-card">
                <div class="tree-card-header tree-item">
                  <div class="list-item-info">
                    ...
                    ${below ? b`<span class="tree-count" title="Devices below this Access Point">${below}</span>` : null}
                  </div>
                  <div class="list-item-actions">
  ...
  _renderChildLists(holderPath, holder, noun = "Switch") {
    ...
    return b`
      ...
      ${this._infoHeader(
        "Servers",
        `Servers added here hang off this ${noun} too. Each keeps its own Containers/VMs, PoE and Connected Devices, and shows any Security badges targeted at Server.`
      )}
      ${homelabs.map((hl, i) => b`
        <div class="list-item">
          <div class="list-item-info">
  ...
  _renderFeedsNote(holder, isAp = false) {
    ...
    if (isAp) {
      ...
      // to the Clients box, and its Connected Devices circle hangs off that.
      return feeds
        ? b`<p class="tree-note">Because this Access Point feeds other devices, its Connected Devices circle hangs off its own dotted line beside them, running down to the Clients box.</p>`
        : null;
    }
    return feeds
      ? b`<p class="tree-note">Not drawn while this feeds other devices - only a device at the end of a branch shows its Connected Devices circle.</p>`
      : null;
  }
  ...
  _renderHomelabFieldsAt(hlPath) {
    ...
    return b`
      ...
      <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
        ...
        this off to hide the line down to Connected Devices and the
  ...
  _scanForRouter() {
    ...
    this._routerScanMessage = candidates.length
      ? ""
      : "No TP-Link Router, Deco master unit, OpenWrt (LuCI), Synology SRM, AsusRouter, UniFi, Omada, or Netgear gateway found.";
    this.requestUpdate();
  }
    ...
    return b`
      <div class="form-section">
        ...
        <div class="toggle-row">
          ${this._toggleLabel(
            "Show Client Names",
            "Shows each client's name in small text directly below its circle. Off by default, since it noticeably increases visual density on a Clients box with a lot of devices - the full name is always available on tap/hover either way."
          )}
          <ha-switch
