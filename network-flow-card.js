...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    let candidates = Array.from(
      branches.querySelectorAll(".bus-top-connector .vline, .bus-top-connector .vline-offline, .bus-top-connector .offline-x-mid")
    );
  ...
  _verticalPassThroughLine(color, duration, animate, dashed = false) {
    return html`
      <div class="vline-single" style="height:100%">
        <div
          class="vline"
          style="${dashed
            ? `left:50%;background-image:repeating-linear-gradient(to bottom, ${color} 0px, ${color} 4px, transparent 4px, transparent 8px);`
  ...
  _renderBranches(columns, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, minDur, maxDur) {
    ...
    const extraCol = showPrimaryExtraCol ? 1 : null;
    const apCol = (i) => {
    ...
    const columnDurations = columns.map((col) => {
      if (col.ap) {
        ...
        return Math.min(dlDur, ulDur);
      }
      return 3;
    });
    // Only a plain AP (no Switch in its node) has a meaningful wired vs
    // wireless backhaul concept - a Switch-only or Switch-group column
    // is always solid, since that represents the switch's own wired
    // port, not a wireless mesh link.
    const columnWired = columns.map((col) => {
      if (col.ap && !col.switch) {
        return isBackhaulWired(hass, col.ap);
      }
      return true;
    });
    ...
    return html`
      <div
        ...
        style="grid-template-columns:repeat(${totalCols}, max-content); grid-template-rows:${collapseTbar ? '0px' : (routerEnabled ? '26px' : '0px')} ${collapseTbar ? '0px' : '2px'} ${switchRowHeight} ${apRowHeight} ${devRowHeight} 24px; column-gap:${this._config.ap_column_gap ?? 32}px; transform:translateX(${fallbackShift}px);"
      >
        ${collapseTbar
          ? null
          : !routerEnabled
        ...
        ${!hasAnySwitch ? null : columns.map((col, i) => {
          const colOffline = routerOffline || (col.switch && isEntityUnavailable(hass, col.switch.entity));
          if (!col.switch) {
            return html`
          ...
          return html`
            <div class="ap-col-line bus-top-connector" style="grid-column:${apCol(i)} / span ${colSpan}; grid-row:3;">
              ${routerOffline
                ? null
                : this._verticalSingleLine(col.switch.colors?.bus_line || "var(--divider-color, #ccc)", 3, animate, 28, false)}
            </div>
            <div
        ...
        ${!hasAnyAp ? null : columns.map((col, i) => {
          ...
          if (!col.ap) {
            ...
            return html`
              <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:5;">
                ${feederOffline ? null : this._verticalPassThroughLine(col.switch?.colors?.bus_line || busLineColor, columnDurations[i], animate)}
              </div>
            `;
          }
          ...
          if (col.switch && col.switchGroupSize > 1 && col.switchGroupStart) {
            ...
            return html`
              <div class="mini-bus-stem" style="grid-column:${apCol(i)} / span ${colSpan}; grid-row:5; background:${col.switch.colors?.bus_line || busLineColor};"></div>
              <div class="mini-bus-line" style="grid-column:${apCol(i)} / span ${colSpan}; grid-row:5; margin-left:${apSizeForInset / 2}px; margin-right:${apSizeForInset / 2}px; background:${col.switch.colors?.bus_line || busLineColor};"></div>
              <div class="ap-col-line ap-col-line-grouped" style="grid-column:${apCol(i)}; grid-row:5;">
                ${apLineOffline ? this._renderXOnlyVertical(24) : this._verticalPassThroughLine(col.switch.colors?.bus_line || busLineColor, columnDurations[i], animate)}
              </div>
            `;
          }
          if (col.switch && col.switchGroupSize > 1 && !col.switchGroupStart) {
          ...
          return html`
            <div class="ap-col-line ${!hasAnySwitch ? "bus-top-connector" : ""}" style="grid-column:${apCol(i)}; grid-row:5;">
              ${hideApOwnLine
                ...
                ? this._renderXOnlyVertical(28)
                : col.switch
                ? this._verticalSingleLine(col.switch.colors?.bus_line || busLineColor, 3, animate, 28, false)
                : apHasBandwidth
                ? this._verticalDualLine(
                    col.ap.colors.download,
                    col.ap.colors.upload,
                    apDlDur,
                    apUlDur,
                    animate,
                    28,
                ...
                : this._verticalSingleLine(
                    col.ap.colors.line || col.ap.colors.download,
                    apDlDur,
                    animate,
                    28,
                    !apWired
          ...
          if (!col.ap) {
            ...
            return swFeederOffline ? null : html`
              <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:6;">
                ${this._verticalPassThroughLine(col.switch?.colors?.bus_line || busLineColor, columnDurations[i], animate)}
              </div>
            `;
          }
          const feederOffline = routerOffline || (col.switch && isEntityUnavailable(hass, col.switch.entity));
          ...
          return html`
            <div class="ap-col-devline" style="grid-column:${apCol(i)}; grid-row:7; ${devLineOffline ? 'display:flex; justify-content:center; align-items:center;' : ''}">
              ${devLineOffline && (devEntity || hasIndividualDevices)
                ...
                : !devLineOffline && (devEntity || hasIndividualDevices)
                ? isSwitchDevices
                  ? html`<div class="ap-col-fillline" style="background:${devColorSource?.devices_line || 'var(--divider-color, #ccc)'}"></div>`
                  : html`<div
                      class="ap-col-fillline"
                      style="background-image:repeating-linear-gradient(to bottom, ${devColorSource?.devices_line || 'var(--divider-color, #ccc)'} 0px, ${devColorSource?.devices_line || 'var(--divider-color, #ccc)'} 2px, transparent 2px, transparent 6px)"
                    ></div>`
                : null}
            </div>
          `;
        ...
        ${!hasAnyDevices ? null : columns.map((col, i) => {
          ...
          const devCircleSize = col.ap ? (this._config.ap_devices_circle_size ?? 56) : (this._config.switch_devices_circle_size ?? 48);
          const devIconSize = col.ap ? (this._config.ap_devices_icon_size ?? 20) : (this._config.switch_devices_icon_size ?? 18);
