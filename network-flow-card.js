...
class NetworkFlowCard extends LitElement {
  ...
  _renderXOnlyVerticalStretch() {
    return html`
      <div class="vline-pair" style="height:100%; justify-content:center;">
        <div class="offline-x-mid offline-x-pulse" style="color:var(--error-color, #f44336)">
          <ha-icon icon="mdi:close"></ha-icon>
        </div>
      </div>
  ...
  _verticalDualLine(c1, c2, d1, d2, animate, height = 32, dashed = false) {
    ...
    return html`
      <div class="vline-pair" style="height:${heightStyle}">
  ...
  _verticalSingleLine(color, duration, animate, height = 24, dashed = false) {
    const heightStyle = typeof height === "string" ? height : `${height}px`;
    return html`
      <div class="vline-single" style="height:${heightStyle}">
        <div
          class="vline"
          style="${dashed
  ...
  _renderBranches(columns, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, minDur, maxDur, primaryOriginalIndex = -1) {
    ...
    const switchLineRowHeight = hasAnySwitch ? "auto" : "28px";
    const switchCircleRowHeight = hasAnySwitch ? "auto" : "0px";
    ...
    return html`
      <div
        ...
        style="grid-template-columns:repeat(${totalCols}, minmax(${maxCircleSize}px, max-content)); grid-template-rows:${collapseTbar ? '0px' : (routerEnabled ? '26px' : '0px')} ${collapseTbar ? '0px' : '2px'} ${switchLineRowHeight} ${switchCircleRowHeight} ${apRowHeight} ${devRowHeight} 24px; column-gap:${this._config.ap_column_gap ?? 32}px; transform:translateX(${fallbackShift}px);"
      >
        ${collapseTbar
          ? null
          : !routerEnabled
          ...
          if (!col.ap) {
            ...
            return html`
              <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:5;">
                ${feederOffline ? null : this._verticalPassThroughLine(busLineColor, columnDurations[i], animate)}
              </div>
            `;
          }
          ...
          if (!col.ap) {
            ...
            return swFeederOffline ? null : html`
              <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:6;">
                ${this._verticalPassThroughLine(busLineColor, columnDurations[i], animate)}
              </div>
            `;
          }
          const feederOffline = routerOffline || (col.switch && isEntityUnavailable(hass, col.switch.entity));
