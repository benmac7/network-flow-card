...
class NetworkFlowCard extends LitElement {
  ...
  _renderDnsBadge(hass, stackIndex = 0) {
    ...
    const state = getEntityState(hass, cfg.dns_entity);
    ...
    const badgeSize = cfg.poe_badge_size ?? 16;
    const fontSize = cfg.poe_badge_font_size ?? 9;
    const hPad = Math.round(badgeSize * 0.35);
    const flashClass =
