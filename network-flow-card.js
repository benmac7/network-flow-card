...
class NetworkFlowCard extends LitElement {
  ...
  render() {
    ...
    // Nodes are the new schema. For now, a node is either AP-only
    // (behaves exactly as before) or Switch-only (adapted into the
    // same AP shape so it flows through the same, already-correct
    // rendering pipeline). Switch-feeding-multiple-APs nodes are a
    // later phase.
...
class NetworkFlowCardEditor extends LitElement {
  ...
  _setPrimaryAp(index, value) {
    const nodes = (this._config.nodes || []).map((node, i) => ({
      ...
      access_points: (node.access_points || []).map((ap, j) => ({
        ...ap,
        is_primary: i === index && j === 0 ? value : false
      }))
    }));
    this._config = { ...this._config, nodes };
    this._fireChanged();
  ...
  _renderApNodeFields(index, node) {
    const ap = node.access_points?.[0] || DEFAULT_ACCESS_POINT;
    const prefix = `nodes.${index}.access_points.0`;
    const c = ap.colors || {};
