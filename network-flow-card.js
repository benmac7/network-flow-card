/**
 * NETWORK-FLOW-CARD v1.1.0-beta.1
...
console.info(
  "%c NETWORK-FLOW-CARD %c v1.1.0-beta.1 ",
...
const DEFAULT_ACCESS_POINT = {
  ...
  colors: {
    ...
    devices_offline_circle: "#e1e1e1",
    devices_offline_icon: "var(--secondary-text-color)"
  }
};
...
class NetworkFlowCard extends LitElement {
  ...
  render() {
    ...
    const routerOfflineColor = routerOfflineTop
      ? router.colors.offline_circle || "#e1e1e1"
    ...
    const primaryApItem = apData.find((a) => a.ap.is_primary) || null;
    ...
    const primaryApOfflineColor = primaryApItem
      ? (primaryApOffline
          ? primaryApItem.ap.colors.offline_circle || "#e1e1e1"
  ...
  _renderTrunk(
    ...
    const primaryApIconColor = primaryApItem
      ? (primaryApOffline
          ? primaryApItem.ap.colors.offline_icon || "var(--secondary-text-color)"
    ...
    const internetCircleColor = internetOffline
      ? internet.colors.offline_circle || "#e1e1e1"
      : internet.colors.circle;
    const internetIconColor = internetOffline
      ? internet.colors.offline_icon || "var(--secondary-text-color)"
      : internet.colors.icon;
    const routerCircleColor = routerOffline
      ? router.colors.offline_circle || "#e1e1e1"
      : router.colors.circle;
    const routerIconColor = routerOffline
      ? router.colors.offline_icon || "var(--secondary-text-color)"
      : router.colors.icon;
    const lanCircleColor = lanOffline
      ? lan.colors.offline_circle || "#e1e1e1"
      : lan.colors.circle;
    const lanIconColor = lanOffline
      ? lan.colors.offline_icon || "var(--secondary-text-color)"
      : lan.colors.icon;
  ...
  _renderBranches(apData, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled) {
    const count = apData.length;
    const collapseTbar = count === 1;
    const hideApOwnLine = !routerEnabled && count === 1;
    return html`
      <div
        class="branches ${hasIndividualDevices ? "with-dev-connectors" : ""}"
        style="grid-template-columns:repeat(${count}, max-content); grid-template-rows:${collapseTbar ? '0px' : (routerEnabled ? '26px' : '0px')} ${collapseTbar ? '0px' : '2px'} auto auto auto auto 24px;"
      >
        ${collapseTbar
          ? null
          : !routerEnabled
        ...
        ${apData.map(
          (item, idx) => {
            ...
            const apCircleColor = apOffline
              ? item.ap.colors.offline_circle || "#e1e1e1"
              : item.ap.colors.circle;
            const apIconColor = apOffline
              ? item.ap.colors.offline_icon || "var(--secondary-text-color)"
              : item.ap.colors.icon;
            return html`
              <div
                class="circle-wrap ap-col-circle"
                style="width:${apSize}px; height:${apSize}px; grid-column:${idx + 1}; opacity:${apOffline ? 0.6 : 1};"
        ...
        ${apData.map(
          (item, i) => {
            ...
            return html`
            <div class="ap-col-devline" style="grid-column:${i + 1}; ${devLineOffline ? 'display:flex; justify-content:center; align-items:center;' : ''}">
        ...
        ${apData.map((item, idx) => {
          ...
          if (item.hasDevices && !apLineOffline) {
            const devCircleColor = devOffline
              ? item.ap.colors.devices_offline_circle || "#e1e1e1"
              : item.ap.colors.devices_circle;
            const devIconColor = devOffline
              ? item.ap.colors.devices_offline_icon || "var(--secondary-text-color)"
              : item.ap.colors.devices_icon;
            return html`
                <div
                  class="circle-wrap ap-col-devcircle"
                  style="width:${devCircleSize}px; height:${devCircleSize}px; grid-column:${idx + 1}; opacity:${devOffline ? 0.6 : 1};"
                  @click=${() =>
          ...
          return hasIndividualDevices && !apLineOffline
            ? html`<div
                ...
                style="grid-column:${idx + 1}; ${devOffline ? 'display:flex; justify-content:center; align-items:center;' : ''}"
              >
                ${devOffline
                  ? html`
                      <div class="offline-x-mid offline-x-pulse" style="color:var(--error-color, #f44336)">
        ...
        ${hasIndividualDevices
          ? apData.map(
              (item, i) => {
                ...
                const devLineOffline = !apLineOffline && item.hasDevices && isEntityUnavailable(hass, item.ap.entities.connected_devices);
                return html`
                  <div class="ap-col-devconnector" style="grid-column:${i + 1}; ${(devLineOffline || apLineOffline) ? 'display:flex; justify-content:center; align-items:center;' : ''}">
                    ${apLineOffline
                      ? null
                      : devLineOffline
                      ? html`
