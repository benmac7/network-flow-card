...
const DEFAULT_CONFIG = {
  ...
  individual_devices_group_padding: 6,
  show_summary: true,
  animation: true,
...
function getDeviceGroupName(hass, dev, groupBy) {
  ...
  if (groupBy === "ap") {
    return attrs.deco_device || "Unknown";
  }
  return "Unknown";
}
    ...
    return html`
      <div class="dev-row-container">
        ...
        <div class="individual-devices-box" style="${groups ? 'padding:8px;' : ''}">
          ...
          ${groups
            ? html`
                <div class="individual-devices-groups">
                  ${groups.map((group) => {
                    const isWired = group.name === "Wired";
                    const radius = config.individual_devices_box_radius || "var(--ha-card-border-radius, 12px)";
                    const pad = config.individual_devices_group_padding ?? 6;
                    return html`
                      <div class="individual-device-group" style="padding:${pad}px; gap:${pad}px;">
                        <svg class="individual-devices-box-border">
                          <rect
                            x="1"
                            y="1"
      ...
      <div class="form-section">
        ...
        <div class="select-field">
          ...
          >
            ...
            <option value="vlan">Network / VLAN</option>
            <option value="ap">Access Point</option>
          </select>
        </div>
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          ...
          "Wired". Otherwise, Network/VLAN groups by the entity's
          "interface" attribute, and Access Point groups by its
          "deco_device" attribute - anything missing that attribute
          falls into "Unknown". Override any individual device's group
