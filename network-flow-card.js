...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    if (!busLine) return;
    // Measure against the circle-wrap elements specifically (they have an
    // explicit inline width set directly, so their rendered box is
    // unambiguous) rather than the line elements above them, whose
    // rendered width depends on grid stretch behavior and isn't reliable
    // to measure directly.
    ...
    const leftMargin = leftMostRect.left + leftMostRect.width / 2 - branchesRect.left;
    const rightMargin = branchesRect.right - (rightMostRect.left + rightMostRect.width / 2);
    busLine.style.marginLeft = `${Math.max(0, leftMargin)}px`;
    busLine.style.marginRight = `${Math.max(0, rightMargin)}px`;
  }
  ...
  _renderBranches(apData, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, primaryBadgeColor, primaryBadgeIconColor, primaryBadgeOutlineColor) {
    ...
    const busLineStyle = `background:${busLineColor}; margin-left:24px; margin-right:24px;`;
    return html`
      <div
