...
// `ookla_speedtest` - adds jitter and ISP). Returns every service found
// on either platform as a separate candidate - a user might have both
// integrations configured, or multiple instances of the same one - so
...
const SPEEDTEST_PLATFORM_LABELS = { speedtestdotnet: "Speedtest.net", ookla_speedtest: "Ookla Speedtest" };
...
class NetworkFlowCardEditor extends LitElement {
  ...
  _renderRouterPage() {
    ...
    return html`
      <div class="form-section">
        ...
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          Looks for both the master unit on a TP-Link Deco mesh and the
          single device from the TP-Link Router integration - if both
          exist (e.g. a TP-Link Router acting as your modem/gateway
          with a Deco mesh riding behind it), you choose which one is
          the Router below rather than having one picked for you.
        </p>
  ...
  _selectSpeedtestService(svc) {
    ...
    this._config = {
      ...
      internet: {
        ...internet,
        name: internet.name || svc.isp || internet.name,
        entities: {
          ...internet.entities,
          ping: internet.entities?.ping || svc.ping,
          jitter: internet.entities?.jitter || svc.jitter,
          download: internet.entities?.download || svc.download,
          upload: internet.entities?.upload || svc.upload
        }
      }
    };
  ...
  _selectIspService(svc) {
    ...
    this._config = {
      ...
      internet: {
        ...internet,
        entity: internet.entity || svc.connected,
        entities: {
          ...internet.entities,
          billing_total: internet.entities?.billing_total || svc.billingTotal,
          billing_remaining: internet.entities?.billing_remaining || svc.billingRemaining,
          total_download: internet.entities?.total_download || svc.totalDownload,
          total_upload: internet.entities?.total_upload || svc.totalUpload
        }
      }
    };
  ...
  _scanForRouter() {
    ...
    this._discoveredRouters = result.routerCandidates;
    this._routerScanMessage = result.routerCandidates.length ? "" : "No TP-Link Router or Deco master unit found.";
    this.requestUpdate();
  }
  ...
  _selectDiscoveredRouter(candidate) {
    this._config = {
      ...
      router: {
        ...
        name: this._config.router.name || candidate.name
      }
    };
  ...
  _renderDiscoveredRoutersList() {
    ...
    const sourceLabel = { tplink_deco: "TP-Link Deco (master)", tplink_router: "TP-Link Router" };
