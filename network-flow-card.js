...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    if (!busLine) return;
    // The T-bar needs to reach the true outer edge of whatever's rendered
    // in the outermost columns - which may be the circle, OR a dual line
    // pair, whichever is wider. Aligning only to the circle's center (as
    // earlier versions did) put the bar in the gap between a dual line's
    // two strands instead of touching either of them, since those
    // strands sit offset from center rather than on it.
    const candidates = Array.from(
      branches.querySelectorAll(
        ".ap-col-circle, .ap-col-devcircle, .ap-col-fillline-wrap, .vline-pair, .vline-single"
      )
    );
