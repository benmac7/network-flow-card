...
function scanTplinkDevices(hass) {
  ...
  if (decoMasterTracker) {
    ...
    // TP-Link Router has no dedicated "status"/"online" entity at all
    // (confirmed against its own sensor.py) - every sensor instead
    // reports unavailable independently based on whether ITS OWN value
    // is present. "Total clients" (key `clients_total`) is the best
    // stand-in: it's in the integration's base sensor set unconditionally
    // (unlike the LTE/VPN/serving-cell sensors, which only appear on
    // matching hardware), so it's present on every install. Buttons are
    // excluded from the fallback entirely - a button's state reflects
    // when it was last pressed, not the router's reachability, so using
    // one here would make the Router node look permanently offline.
    const onDevice = all.filter((e) => e.deviceId === routerDeviceId && e.domain !== "button");
    const preferred = onDevice.find((e) => e.domain === "sensor" && e.entityId.endsWith("total_clients"));
  ...
  // --- Node candidates: every Deco unit that ISN'T the master ---
  const nodes = decoUnitTrackers
    .filter((e) => !tplinkIsDecoMaster(hass, e.entityId))
    .map((e) => {
...
// Start.ca (platform `startca`) - checked in that order, first match
// wins. Field names below are confirmed straight from each
...
function scanIspIntegration(hass) {
  ...
  const platform = ISP_PLATFORMS.find((p) => byPlatform[p]?.length);
  if (!platform) return null;
...
class NetworkFlowCardEditor extends LitElement {
  ...
  _renderInternetPage() {
    ...
    return html`
      <div class="form-section">
        ...
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          Looks for Aussie Broadband, Starlink, or Start.ca (checked in
          that order) and fills in Billing Total, Billing Remaining,
          Total Downloaded, and Total Uploaded below where available.
          For Starlink, also sets the Internet Entity itself to its
          "Connected" binary sensor if that field is still blank. Only
  ...
  _scanForIsp() {
    const result = scanIspIntegration(this.hass);
    if (!result || (!result.connected && !result.billingTotal && !result.billingRemaining && !result.totalDownload && !result.totalUpload)) {
      this._ispScanMessage = "No Aussie Broadband, Starlink, or Start.ca entities found.";
      this.requestUpdate();
      return;
    }
    ...
    this._config = {
      ...
      internet: {
        ...internet,
        entity: internet.entity || result.connected,
        entities: {
          ...internet.entities,
          billing_total: internet.entities?.billing_total || result.billingTotal,
          billing_remaining: internet.entities?.billing_remaining || result.billingRemaining,
          total_download: internet.entities?.total_download || result.totalDownload,
          total_upload: internet.entities?.total_upload || result.totalUpload
        }
      }
    };

    const platformLabel = { aussie_broadband: "Aussie Broadband", starlink: "Starlink", startca: "Start.ca" }[result.platform] || result.platform;
    const found = [
      result.connected && "Internet entity",
      result.billingTotal && "billing total",
      result.billingRemaining && "billing remaining",
      result.totalDownload && "total downloaded",
      result.totalUpload && "total uploaded"
    ].filter(Boolean);
    this._ispScanMessage = found.length
      ? `Found via ${platformLabel}: ${found.join(", ")}.`
      : `Found ${platformLabel} but no matching fields - check Developer Tools > States on your own entities.`;
    this._fireChanged();
  }
    ...
    return html`
      <div class="form-section">
        ...
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          ...
          icon, color, name, and group are all still yours to set
          afterward.
        </p>
  ...
  // Entity picker handler for an Individual Device: behaves like the
