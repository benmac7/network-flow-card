...
function normalizeFedSwitch(sw) {
  ...
  merged.access_points = ((sw && sw.access_points) || []).map((ap) =>
...
function computeDiagramPoeTotal(hass, config) {
  ...
  (config.nodes || []).forEach((node) => {
    ...
    (node.homelabs || []).forEach((hl) => {
      if (hl.poe && hl.poe.mode !== "off") switches.push(hl);
    });
    const walkFed = (sw) => {
      if (sw.poe && sw.poe.mode !== "off") switches.push(sw);
      (sw.access_points || []).forEach((ap) => {
        if (ap.poe && ap.poe.mode !== "off") switches.push(ap);
      });
      (sw.homelabs || []).forEach((hl) => {
        if (hl.poe && hl.poe.mode !== "off") switches.push(hl);
      });
      (sw.fed_switches || []).forEach(walkFed);
    };
    (node.fed_switches || []).forEach(walkFed);
  });
...
function buildNetworkTopology(config) {
  ...
  // Every Switch - a Node's own, or a fed switch - feeds its Access
  // Points first, then its fed switches, then its Homelab servers, in
  // that order. A fed switch may itself feed more of any, to any depth;
  // Access Points and Homelabs are always the end of a branch.
  ...
  const collect = (children, tier, parentCell, nodeSwitch, chainSoFar) => {
    children.forEach((child, sibIndex) => {
      ...
      chain[tier] = cell;
      const grandChildren = child.kind === "fedswitch" ? childrenOf(child.cfg) : [];
  ...
  (config.nodes || []).forEach((node) => {
    ...
    if (node.switch && kids.length) {
      ...
      (node.access_points || []).forEach((ap) => {
        const cell = {
          tier: 1, kind: "ap", cfg: ap, parent: null, sibIndex: 0, sibCount: 1,
          firstCol: null, span: 1, childCount: 0, isTerminal: true
        };
        const col = { ap, subSwitch: null, switch: null, switchGroupStart: false, switchGroupSize: 0, termTier: 1, chain: [null, cell] };
        cell.firstCol = col;
        columns.push(col);
      });
    }
...
class NetworkFlowCard extends i {
  ...
  _renderBranches(columns, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, minDur, maxDur, primaryOriginalIndex = -1, tierCount = 2) {
    ...
    const columnHasDevices = (c) => (c.isHomelab && c.switch.show_devices_line === false) || (c.homelab && c.homelab.show_devices_line === false)
      ? false
      : (c.subSwitch ? !!c.subSwitch.entities.connected_devices : c.homelab ? !!c.homelab.entities?.connected_devices : c.ap ? !!c.ap.entities.connected_devices : !!(c.switch && c.switch.entities.connected_devices));
    ...
    const kindSize = (kind) => (kind === "ap" ? apSizeCfg : kind === "homelab" ? hlSizeCfg : swSizeCfg);
    ...
    const infoFor = (cell, col) => {
      ...
      const upstream = parent && parent.kind === "fedswitch"
    ...
    const cellLine = (t, i, col, cell) => {
      ...
      const dur = cell.isTerminal ? columnDurations[i] : 3;
      ...
      if (parent && parent.childCount > 1) {
        ...
        return b`
          ${first ? b`<div class="mini-bus-stem" style="grid-column:${gridCol(parent)}; grid-row:${row}; background:${busLineColor};"></div>` : null}
          <div class="ap-col-line ap-col-line-grouped" style="grid-column:${cs}; grid-row:${row};">
            ...
            ${offline ? this._renderXOnlyVerticalStretch() : this._verticalPassThroughLine(busLineColor, dur, animate)}
          </div>
        `;
      }
      ...
      if (parent) {
        ...
        return b`
          <div class="ap-col-line" style="grid-column:${cs}; grid-row:${row};">
      ...
      return b`
        <div class="ap-col-line ${!hasAnySwitch ? "bus-top-connector" : ""}" style="grid-column:${cs}; grid-row:${row};">
          ${hideApOwnLine
            ...
            ? this._renderXOnlyVerticalStretch()
            : apHasBandwidth
            ? this._verticalDualLine(busLineColor, busLineColor, vm.dlDur, vm.ulDur, animate, "100%", !vm.wired)
            : this._verticalSingleLine(busLineColor, vm.dlDur, animate, "100%", !vm.wired)}
    ...
    // line carrying on down toward its Connected Devices circle and/or the
    ...
    const belowLine = (t, i, col) => {
      ...
      const gap = t === col.termTier + 1 ? Math.max(0, (tierRowMax[col.termTier] || own) - own) : 0;
      ...
      return b`
        <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:${row};">
          <div class="ap-col-fillline" style="${col.ap
    ...
    const fedCircle = (t, i, col, cell) => {
      ...
      return b`
        <div
          ...
          @click=${() => this._handleMoreInfo(sw.entity)}
        >
          ...
          ${this._renderPoeBadge(hass, sw.entity, sw.poe)}
        </div>
      `;
    };
    ...
    return b`
      ...
      >
        ...
        ${!hasAnyDevices ? null : columns.map((col, i) => {
          ...
          const homelabHidesLine = vm.homelabHidesLine;
          const devEntity = vm.devEntity;
          const devColorSource = col.subSwitch ? col.subSwitch.colors : col.homelab ? col.homelab.colors : col.ap ? col.ap.colors : col.switch?.colors;
          const devIcon = col.subSwitch ? col.subSwitch.devices_icon : col.homelab ? col.homelab.devices_icon : col.ap ? col.ap.devices_icon : col.switch?.devices_icon;
          const devCircleSize = this._config.ap_devices_circle_size ?? 56;
...
class NetworkFlowCardEditor extends i {
  ...
  _getPageTitle() {
    ...
    if (this._page === "nodes" && this._editingApIndex !== null && this._editingChildPath.length) {
      ...
      return key === "access_points" ? `Access Point ${n}` : key === "homelabs" ? `Homelab ${n}` : `Switch ${n}`;
    }
  ...
  _renderHomelabSecurity(hlPath, hl) {
    ...
    if (!active.length) {
      return b`
        ...
        <p class="tree-note">
          ...
          at Homelab Servers from the Security page. Once one is, choose
          here which servers run it.
        </p>
      `;
    }
    ...
    return b`
      ...
      <p class="tree-note">
        ...
        ${explicit ? "" : "Until you change one, every Homelab shows all of them."}
      </p>
      ${active.map((item) => b`
        <div class="toggle-row">
  ...
  _allAccessPoints() {
    const out = [];
    const walk = (holder) => {
      (holder.access_points || []).forEach((ap) => out.push(ap));
  ...
  _renderHomelabTargetHint(item) {
    ...
    const names = homelabs
      ...
      .map((hl) => this._treeLabel(hl, "Homelab"));
    return b`
      <p class="tree-note">
        ${names.length ? `Shown on: ${names.join(", ")}.` : "Not shown on any Homelab yet."}
        Choose which servers run it from each Homelab's page.
      </p>
    `;
  }
    ...
    return b`
      <div class="form-section">
        ...
        ${nodes.map((node, idx) => {
          ...
          const holderPath = `nodes.${idx}`;
          const expanded = isSwitchNode && this._expanded.has(holderPath);
          const below = isSwitchNode ? this._countBelow(node) : 0;
            ...
            <div class="tree-node">
              <div class="list-item tree-item">
                <div class="list-item-info">
                  ${isSwitchNode ? this._treeChevron(holderPath, expanded) : b`<span class="tree-chevron-spacer"></span>`}
                  ...
                  ${below ? b`<span class="tree-count" title="Devices below this Switch">${below}</span>` : null}
                </div>
                <div class="list-item-actions">
                  <ha-icon-button
              ...
              ${expanded ? this._renderTreeChildren(holderPath, node) : null}
            </div>
          `;
        })}
  ...
  _setPrimaryApAt(prefix, value) {
    const clearHolder = (holder) => ({
      ...holder,
      access_points: (holder.access_points || []).map((ap) => ({ ...ap, is_primary: false })),
  ...
  _renderApEditor(index) {
    ...
    if (isSwitchNode && this._editingChildPath.length) {
      return b`
        <div class="form-section">
  ...
  _countBelow(holder) {
    let n = (holder.access_points || []).length + (holder.homelabs || []).length;
  ...
  _childSegments(holderPath, key, idx) {
    const rest = holderPath.split(".").slice(2);
    const segs = [];
  ...
  _removeChildFrom(holderPath, key, idx) {
    ...
    const below = target && key === "fed_switches" ? this._countBelow(target) : 0;
    if (below && !window.confirm(`Delete this Switch and the ${below} device${below === 1 ? "" : "s"} below it?`)) return;
  ...
  _renderTreeChildren(holderPath, holder) {
    ...
    return b`
      <div class="tree-children">
        ${aps.map((ap, i) => b`
          <div class="list-item tree-item">
            <div class="list-item-info">
              ...
              <span class="tree-label">${this._treeLabel(ap, `AP ${i + 1}`)}${ap.is_primary ? " (Primary)" : ""}</span>
            </div>
            <div class="list-item-actions">
            ...
            <div class="tree-node">
              <div class="list-item tree-item">
                <div class="list-item-info">
                  ...
                  <span class="tree-label">${this._treeLabel(sw, `Switch ${i + 1}`)}</span>
                  ${below ? b`<span class="tree-count" title="Devices below this Switch">${below}</span>` : null}
                </div>
                <div class="list-item-actions">
              ...
              ${expanded ? this._renderTreeChildren(path, sw) : null}
            </div>
          `;
        })}
        ${homelabs.map((hl, i) => b`
          <div class="list-item tree-item">
            <div class="list-item-info">
              ...
              <span class="tree-label">${this._treeLabel(hl, `Homelab ${i + 1}`)}</span>
            </div>
            <div class="list-item-actions">
        ...
        <div class="tree-add-row">
          <button class="tree-add-btn" @click=${() => this._addChildTo(holderPath, "access_points")}>+ Access Point</button>
          <button class="tree-add-btn" @click=${() => this._addChildTo(holderPath, "fed_switches")}>+ Switch</button>
          <button class="tree-add-btn" @click=${() => this._addChildTo(holderPath, "homelabs")}>+ Homelab</button>
        </div>
      </div>
    `;
  }
  ...
  _renderChildLists(holderPath, holder) {
    const aps = holder.access_points || [];
    const feds = holder.fed_switches || [];
    const homelabs = holder.homelabs || [];
    return b`
      ...
      ${this._infoHeader(
        "Fed Switches",
        "Switches added here hang off this Switch too, in the same row as its Access Points. A fed Switch can feed its own Access Points and Switches in turn, to any depth."
      )}
      ${feds.map((sw, i) => b`
        <div class="list-item">
          <div class="list-item-info">
      ...
      ${this._infoHeader(
        "Homelab Servers",
        "Homelab servers added here hang off this Switch too, in the same row as its Access Points. Each keeps its own Containers/VMs, PoE and Connected Devices, and shows any Security badges targeted at Homelab."
      )}
      ${homelabs.map((hl, i) => b`
        <div class="list-item">
          <div class="list-item-info">
            <ha-icon .icon=${hl.icon || "mdi:server"} style="color:var(--primary-color);"></ha-icon>
            <span>${hl.name || hl.entity || `Homelab ${i + 1}`}</span>
          </div>
          <div class="list-item-actions">
            <ha-icon-button @click=${() => this._openChild(holderPath, "homelabs", i)} title="Edit">
              <ha-icon icon="mdi:pencil"></ha-icon>
  ...
  // Connected Devices circle, so say so instead of silently ignoring it.
  _renderFeedsNote(holder) {
    const feeds = (holder.access_points || []).length + (holder.fed_switches || []).length + (holder.homelabs || []).length;
    return feeds
      ? b`<p class="tree-note">Not drawn while this Switch feeds other devices - only a Switch at the end of a branch shows its Connected Devices circle.</p>`
      : null;
  }
  ...
  _renderBreadcrumb(nodeIndex) {
    ...
    const parts = [node.name || node.switch?.name || node.switch?.entity || `Node ${nodeIndex + 1}`];
    let prefix = `nodes.${nodeIndex}`;
    this._editingChildPath.forEach((seg) => {
      prefix += `.${seg}`;
      parts.push(this._treeLabel(this._getAtPath(prefix), seg.startsWith("access_points") ? "Access Point" : seg.startsWith("homelabs") ? "Homelab" : "Switch"));
    });
    return b`<div class="tree-breadcrumb">${parts.join(" › ")}</div>`;
  }

  _renderChildEditor(nodeIndex) {
    const prefix = `nodes.${nodeIndex}.${this._editingChildPath.join(".")}`;
  ...
  _removeAp(idx) {
    ...
    const below = target ? this._countBelow(target) : 0;
