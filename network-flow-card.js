/**
 * NETWORK-FLOW-CARD v1.4.0-beta.1
...
console.info(
  "%c NETWORK-FLOW-CARD %c v1.4.0-beta.1 ",
...
class NetworkFlowCard extends LitElement {
  ...
  _renderBranches(apData, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout) {
    ...
    const totalCols = count + (primaryHasDevices ? 1 : 0);
    const midCol = primaryHasDevices ? Math.ceil((count + 1) / 2) : null;
    const extraCol = midCol;
    const apCol = (i) => {
      if (!primaryHasDevices) return i + 1;
      return (i + 1) < midCol ? (i + 1) : (i + 2);
    };
    const collapseTbar = count === 1 && !primaryHasDevices;
    const hideApOwnLine = !routerEnabled && count === 1 && !primaryHasDevices;

    // The bus-line's endpoints need to align with the center of whatever
    // sits in the first/last column - which may be an AP (whose column
    // width depends on its circle, devices circle, and whether its line
    // is single or dual) or, at an edge, the primary's devices column.
    const edgeColWidth = (item) => {
      if (!item) return 72;
      const circleW = item.ap.circle_size ?? 72;
      const lineW = (item.ap.entities.download || item.ap.entities.upload) ? 48 : 8;
      const devW = item.hasDevices ? (item.ap.devices_circle_size ?? 56) : 0;
      return Math.max(circleW, lineW, devW);
    };
    const primaryDevW = primaryHasDevices ? (primaryApItem.ap.devices_circle_size ?? 56) : 72;
    const firstIsPrimaryDev = primaryHasDevices && midCol === 1;
    const lastIsPrimaryDev = primaryHasDevices && midCol === totalCols;
    const leftEdgeWidth = firstIsPrimaryDev ? primaryDevW : edgeColWidth(apData[0]);
    const rightEdgeWidth = lastIsPrimaryDev ? primaryDevW : edgeColWidth(apData[apData.length - 1]);
    const busMarginLeft = Math.max(4, leftEdgeWidth / 2 - 8);
    const busMarginRight = Math.max(4, rightEdgeWidth / 2 - 8);
    const busLineStyle = `background:${busLineColor}; margin-left:${busMarginLeft}px; margin-right:${busMarginRight}px;`;
    return html`
      <div
      ...
      >
        ${collapseTbar
          ...
          : !routerEnabled
          ? html`
              <div class="bus-line ${count === 1 ? 'single' : ''}" style="${busLineStyle}"></div>
            `
          : routerOffline
          ? html`
            ...
            return html`
              ...
              >
                ...
                ${primaryApLayout === "flat" && item.ap.is_primary
                  ? html`<div class="primary-ap-flat-badge">
          ...
          ? (() => {
              ...
              return html`
                <div
                  ...
                  style="grid-column:${extraCol}; grid-row:3 / 6; display:flex; justify-content:center;"
                >
                  <div
                    class="ap-col-fillline"
