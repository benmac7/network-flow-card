...
const DEFAULT_CONFIG = {
  ...
  individual_devices_group_by: "ssid",
...
class NetworkFlowCard extends i {
  ...
  _renderIndividualDevices(devices, hass, config, animate, minDur, maxDur, apCount) {
    ...
    const groupBy = config.individual_devices_group_by || "ssid";
  ...
  _backhaulBadge(ap, hass) {
    ...
    // This used to read hass.states[entityId].state directly, which
    // duplicated (and diverged from) isBackhaulWired: it never checked
    // ap.is_primary (so an auto-discovered Primary AP - whose
    // backhaul_type self-references its own tracker entity, reporting
    // home/not_home rather than "wired" - showed a wifi icon), and it
    // never checked the connection_type ATTRIBUTE some integrations
    // use instead of a dedicated entity. Delegating to the shared,
    // already-correct helper fixes both.
    const entityId = ap.entities?.backhaul_type;
    const hasEntity = !!entityId && !!hass?.states?.[entityId];
    if (!hasEntity && !ap.is_primary) return null;
...
class NetworkFlowCardEditor extends i {
  ...
  // Every candidate carries `type` ("ap" for a TP-Link Deco satellite,
  // "homelab" for a Proxmox node - "switch" is reserved for a future
  // scanner and always offered as a filter option even with zero
  // results today) and `source` (the owning integration), plus `raw`
  // holding whatever type-specific data _addSelectedNodes needs to
