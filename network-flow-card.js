...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    // Prefer measuring the actual visible line content directly - these
    // are absolutely positioned with fixed pixel offsets, so their
    // position is unambiguous. Fall back to the wide row-3 containers
    // (always rendered, explicit width:100%) if fewer than 2 are found.
    ...
    // Bus-line's own reach: touch the actual edges when using precise
    // strokes; touch the centers (not edges) when falling back to wide
    // containers, since a wide container's own edge can coincide with
    // the grid's edge (the 1.4.6 overshoot bug).
    ...
    // Whole-block shift: align the feeder circle above (Router or
    // Primary AP) with the MIDPOINT BETWEEN THE TWO OUTERMOST COLUMNS'
    // OWN CENTERS - not the whole block's geometric centroid. Those two
    // only coincide when both columns are similar widths. When Primary's
    // own column shrinks to just a thin connecting line (no Connected
    // Devices circle) next to a full-width AP column, the geometric
    // centroid gets pulled hard toward the wider column, while the true
    // midpoint between the two branches stays roughly balanced - using
    // the centroid was skewing the whole block toward whichever column
    // was wider. Deriving this from the SAME leftMost/rightMost pair
    // used for the bus-line above guarantees the two can never disagree
    // with each other.
  ...
  _renderBranches(apData, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, primaryBadgeColor, primaryBadgeIconColor, primaryBadgeOutlineColor, primaryBadgeIcon, primaryBadgeSize, primaryBadgeIconSize) {
