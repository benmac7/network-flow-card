...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    // If that strict selector doesn't find at least two elements in some
    // configuration, fall back to broader container elements rather than
    // silently skipping the correction entirely - a silent skip here
    // means branches.style.transform stays reset to "", reverting to
    // CSS's own (previously shown to be unreliable) grid centering,
    // which produces exactly the kind of "moves left" regression seen
    // when Primary AP has no Connected Devices entity of its own.
    if (candidates.length < 2) {
      candidates = Array.from(
        branches.querySelectorAll(
          ".ap-col-circle, .ap-col-devcircle, .ap-col-fillline-wrap"
        )
      );
    }
