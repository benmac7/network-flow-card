...
class NetworkFlowCardEditor extends LitElement {
  ...
  _selectDiscoveredRouter(candidate) {
    ...
    const lan = this._config.lan || {};
    const incoming = { entity: candidate.entity, name: candidate.name, lan_entity: candidate.lanEntity };
    const existing = { entity: current.entity, name: current.name, lan_entity: lan.entity };
    const conflicts = findFieldConflicts(existing, incoming);
    if (
      conflicts.length &&
