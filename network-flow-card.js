...
class NetworkFlowCard extends LitElement {
  ...
  render() {
    ...
    // Nodes are the new schema (each node can eventually contain a
    // switch and/or multiple APs), but for now every node is still
    // AP-only post-migration, so flattening to one AP per node
    // reproduces the exact same list this card has always worked with.
    const flatApsFromNodes = (config.nodes || []).flatMap((node) => node.access_points || []);
    ...
    return html`
      <div class="form-section">
        ${nodes.map((node, idx) => {
          ...
          const label = node.name || ap.name || ap.entity || `Node ${idx + 1}`;
          return html`
            <div class="list-item">
