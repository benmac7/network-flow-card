...
function computeDiagramPoeTotal(hass, config) {
  const switches = [];
  if (config.switch?.entity) switches.push(config.switch);
  (config.nodes || []).forEach((node) => {
    if (node.switch?.entity) switches.push(node.switch);
  });
...
class NetworkFlowCard extends LitElement {
  ...
  _renderGuestBadge(hass, entityId, online) {
    ...
    return html`<div class="guest-badge" style="background:${bg};">
