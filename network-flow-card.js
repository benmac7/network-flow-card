...
class NetworkFlowCard extends LitElement {
  ...
  _verticalPassThroughLine(color, duration, animate) {
    return html`
      <div class="vline-single" style="height:100%">
        <div class="vline" style="left:50%;background:${color}"></div>
        ${animate ? this._cssDotsY(color, duration, "50%", false, 2) : null}
      </div>
    `;
  }
            ...
            return html`
              <div class="ap-col-line bus-top-connector" style="grid-column:${apCol(i)}; grid-row:3;">
                ${hideApOwnLine || routerOffline
                  ? null
                  : this._verticalPassThroughLine(busLineColor, 3, false)}
              </div>
            `;
          }
          ...
          if (!col.ap) {
            ...
            return html`
              <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:5;">
                ${feederOffline ? null : this._verticalPassThroughLine(col.switch?.colors?.bus_line || busLineColor, 3, false)}
              </div>
            `;
          }
          ...
          if (col.switch && col.switchGroupSize > 1 && col.switchGroupStart) {
            ...
            return html`
              ...
              <div class="ap-col-line ap-col-line-grouped" style="grid-column:${apCol(i)}; grid-row:5;">
                ${apLineOffline ? this._renderXOnlyVertical(24) : this._verticalPassThroughLine(col.switch.colors?.bus_line || busLineColor, 3, animate)}
              </div>
            `;
          }
          if (col.switch && col.switchGroupSize > 1 && !col.switchGroupStart) {
          ...
          if (!col.ap) {
            ...
            return swFeederOffline ? null : html`
              <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:6;">
                ${this._verticalPassThroughLine(col.switch?.colors?.bus_line || busLineColor, 3, false)}
              </div>
            `;
          }
          const feederOffline = routerOffline || (col.switch && isEntityUnavailable(hass, col.switch.entity));
          ...
          return html`
            <div class="ap-col-devline" style="grid-column:${apCol(i)}; grid-row:7; ${devLineOffline ? 'display:flex; justify-content:center; align-items:center;' : ''}">
              ${devLineOffline && (devEntity || hasIndividualDevices)
                ...
                : !devLineOffline && (devEntity || hasIndividualDevices)
                ? isSwitchDevices
                  ? html`<div class="ap-col-fillline" style="background:${devColorSource?.devices_line}"></div>`
                  : html`<div
                      class="ap-col-fillline"
                      style="background-image:repeating-linear-gradient(to bottom, ${devColorSource?.devices_line} 0px, ${devColorSource?.devices_line} 2px, transparent 2px, transparent 6px)"
                    ></div>`
                : null}
            </div>
          `;
