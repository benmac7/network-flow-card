...
const DEFAULT_HOMELAB = {
  ...
  // Devices / the shared Individual Devices box be turned off
...
const DEFAULT_NODE = {
  ...
  // Connected Devices circle to Individual Devices, matching how any
...
const DEFAULT_CONFIG = {
  ...
  // "none" | "vlan" | "ap" - splits the Individual Devices box into
...
// Determines which Group By sub-group an Individual Device belongs
...
function getDeviceGroupName(hass, dev, groupBy) {
  ...
  if (attrs.connection === "wired") return "Wired";
...
// access point on the mesh), and Individual Devices (every remaining
...
// `model` field the integration sets to the real hardware name
// ("Deco X60", etc). If your installed integration version doesn't
// populate one of these the same way, the classification below may
// need adjusting - check Settings > Devices & Services > (the
// integration) to see what your own registry actually looks like.
const TPLINK_PLATFORMS = ["tplink_deco", "tplink_router"];
const TPLINK_INFRA_MODEL_HINTS = ["deco", "archer", "router", "omada"];
...
function tplinkIsInfraDevice(hass, deviceId) {
  const model = (hass.devices?.[deviceId]?.model || "").toLowerCase();
  return TPLINK_INFRA_MODEL_HINTS.some((hint) => model.includes(hint));
}

// Per the TP-Link Deco integration's own documented behavior,
// `deco_device`/`deco_mac` are set on a unit's own tracker entity for
// every NON-master unit - the master's own tracker entity has neither.
function tplinkIsDecoMaster(hass, entityId) {
  const attrs = hass.states?.[entityId]?.attributes || {};
  return attrs.deco_device === undefined && attrs.deco_mac === undefined;
}
...
// but still works out of the box for most installs).
function tplinkFindSibling(hass, entityIds, suffixes) {
  for (const eid of entityIds) {
    const key = hass.entities?.[eid]?.translation_key;
    if (key && suffixes.includes(key)) return eid;
  }
  for (const eid of entityIds) {
    if (suffixes.some((s) => eid.endsWith(s))) return eid;
  }
  return "";
}

function scanTplinkDevices(hass) {
  if (!hass?.entities || !hass?.devices) return { router: null, nodes: [], individualDevices: [] };
  ...
  const infraDeviceIds = new Set(
    trackers.filter((e) => e.deviceId && tplinkIsInfraDevice(hass, e.deviceId)).map((e) => e.deviceId)
  );

  // --- Router / Gateway candidate ---
  let router = null;
  const decoMaster = trackers.find(
    (e) => e.platform === "tplink_deco" && infraDeviceIds.has(e.deviceId) && tplinkIsDecoMaster(hass, e.entityId)
  );
  if (decoMaster) {
    router = {
      entity: decoMaster.entityId,
      deviceId: decoMaster.deviceId,
      name: hass.devices[decoMaster.deviceId]?.name_by_user || hass.devices[decoMaster.deviceId]?.name || "",
      source: "tplink_deco"
    };
  } else {
    const routerDeviceId = [...infraDeviceIds].find((id) =>
      all.some((e) => e.deviceId === id && e.platform === "tplink_router")
    );
    if (routerDeviceId) {
      const onDevice = all.filter((e) => e.deviceId === routerDeviceId);
      const statusLike = onDevice.find((e) => /status|online/.test(e.entityId));
      const chosen = statusLike || onDevice[0];
      router = {
  ...
  // --- Node candidates: every infra device other than whichever one
  // was picked as Router/Gateway above ---
  const nodeDeviceIds = [...infraDeviceIds].filter((id) => id !== router?.deviceId);
  const nodes = nodeDeviceIds.map((deviceId) => {
    const tracker = trackers.find((e) => e.deviceId === deviceId);
    const siblings = deviceEntityMap[deviceId] || [];
    return {
      deviceId,
      entity: tracker?.entityId || "",
      name: hass.devices[deviceId]?.name_by_user || hass.devices[deviceId]?.name || "",
      backhaulType: tplinkFindSibling(hass, siblings, ["connection_type", "connection"]),
      download: tplinkFindSibling(hass, siblings, ["download_speed"]),
      upload: tplinkFindSibling(hass, siblings, ["upload_speed"]),
      connectedDevices: tplinkFindSibling(hass, siblings, ["client_count", "connected_devices", "clients"])
    };
  });

  // --- Individual device candidates: every device_tracker on these
  // platforms whose device ISN'T classified as infra above ---
  const individualDevices = trackers
    .filter((e) => !infraDeviceIds.has(e.deviceId))
    .map((e) => ({
      entity: e.entityId,
  ...
  return { router, nodes, individualDevices };
}

// --- Speedtest Auto-Discovery ---------------------------------------
// Looks for either the official core "Speedtest.net" integration
// (platform `speedtestdotnet` - ping/download/upload only, no jitter
// or ISP) or the community "Ookla Speedtest" integration (platform
// `ookla_speedtest` - adds jitter and ISP). When both are present,
// Ookla Speedtest is preferred since it's the richer of the two.
...
// first.
const SPEEDTEST_PLATFORMS = ["speedtestdotnet", "ookla_speedtest"];
...
function scanSpeedtestIntegration(hass) {
  ...
  const byPlatform = { speedtestdotnet: [], ookla_speedtest: [] };
  for (const [entityId, entry] of Object.entries(hass.entities)) {
    if (SPEEDTEST_PLATFORMS.includes(entry.platform)) byPlatform[entry.platform].push(entityId);
  }

  const platform = byPlatform.ookla_speedtest.length
    ? "ookla_speedtest"
    : byPlatform.speedtestdotnet.length
    ? "speedtestdotnet"
    : null;
  if (!platform) return null;

  const entityIds = byPlatform[platform];
  const deviceEntityMap = tplinkBuildDeviceEntityMap(hass);
  const deviceIds = new Set(entityIds.map((id) => hass.entities[id]?.device_id).filter(Boolean));
  const pool = [...new Set([...entityIds, ...[...deviceIds].flatMap((id) => deviceEntityMap[id] || [])])];
      ...
      >
        ...
        ${!hasApRow ? null : columns.map((col, i) => {
          ...
          if (!col.ap) {
            ...
            // Individual Devices box) - otherwise the line has nowhere
          ...
          return html`
            ...
            >
              ...
              ${(() => {
                ...
                if (!ap.is_primary) {
                  return this._renderPoeBadge(hass, ap.entity, ap.poe, 0);
                }
                const vpnVisible = this._config.vpn_target === "primary_ap" && !!this._config.vpn_entity;
                const fwVisible = this._config.firewall_target === "primary_ap" && !!this._config.firewall_entity;
                const dnsVisible = this._config.dns_target === "primary_ap" && !!this._config.dns_entity;
...
const MENU_ITEMS = [
  ...
  { key: "individual_devices", title: "Individual Devices", icon: "mdi:devices", summary: "Device icons & colors, grouping by SSID/VLAN/AP" },
...
class NetworkFlowCardEditor extends LitElement {
    ...
    return html`
      <div class="form-section">
        ...
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          Looks for the official Speedtest.net integration or the
          community Ookla Speedtest integration (preferred when both
          are present, since it also reports jitter and ISP) and fills
          in Ping, Jitter, Download, and Upload below, plus the Name
          Override with your ISP if it's not already set. Only blank
          fields are filled in - anything you've already set by hand is
          left alone.
        </p>
  ...
  _renderRouterPage() {
    ...
    return html`
      <div class="form-section">
        ...
        <div class="sub-header">Auto-Discovery (TP-Link)</div>
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          Looks for the master unit on a TP-Link Deco mesh, or the
          single device from the TP-Link Router integration, and fills
          in the entity below. Safe to run again later - it only
          overwrites the Router entity, never anything else on this
          page.
        </p>
  ...
  _renderAccessPointsPage() {
    ...
    return html`
      <div class="form-section">
        ...
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          Finds each satellite Deco unit on your mesh and adds it as a
          Node, wiring up Connected Devices, Backhaul Type, and
          Download/Upload from that unit's own sibling entities
          automatically. Units already added (or already used as your
          Router) are skipped, so this is safe to run again after
          adding a new Deco to your mesh.
        </p>
        <button class="add-btn" @click=${() => this._scanForNodes()}>
          Scan for Nodes
        </button>
        ${this._nodeScanMessage
          ? html`<p style="color:var(--secondary-text-color); font-size:0.9em; margin:4px 0 12px;">${this._nodeScanMessage}</p>`
          : null}
  ...
  _renderHomelabFields(index, node) {
    ...
    return html`
      ...
      <div class="sub-header">Connected Devices</div>
      <div class="toggle-row">
        <span>Show connection to Connected/Individual Devices</span>
        <ha-switch
      ...
      <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
        ...
        the shared Individual Devices box entirely.
      </p>
  ...
  _scanForInternet() {
    const result = scanSpeedtestIntegration(this.hass);
    if (!result || (!result.ping && !result.jitter && !result.download && !result.upload && !result.isp)) {
      this._internetScanMessage = "No Speedtest.net or Ookla Speedtest entities found.";
      this.requestUpdate();
      return;
    }
    ...
    this._config = {
      ...
      internet: {
        ...internet,
        name: internet.name || result.isp || internet.name,
        entities: {
          ...internet.entities,
          ping: internet.entities?.ping || result.ping,
          jitter: internet.entities?.jitter || result.jitter,
          download: internet.entities?.download || result.download,
          upload: internet.entities?.upload || result.upload
        }
      }
    };
    ...
    this._internetScanMessage = `Found via ${result.platform === "ookla_speedtest" ? "Ookla Speedtest" : "Speedtest.net"}: ${found.join(", ")}.`;
    this._fireChanged();
  }

  _scanForRouter() {
    const result = scanTplinkDevices(this.hass);
    if (!result.router?.entity) {
      this._routerScanMessage = "No TP-Link Router or Deco master unit found.";
      this.requestUpdate();
      return;
    }
    this._config = {
      ...
      router: {
        ...this._config.router,
        entity: result.router.entity,
        name: this._config.router.name || result.router.name
      }
    };
    this._routerScanMessage = `Set Router entity to ${result.router.entity}${result.router.name ? ` (${result.router.name})` : ""}.`;
    this._fireChanged();
  }
  ...
  _scanForNodes() {
    ...
    (this._config.nodes || []).forEach((n) => {
      (n.access_points || []).forEach((ap) => ap.entity && existingEntities.add(ap.entity));
    });
    if (this._config.router?.entity) existingEntities.add(this._config.router.entity);

    const toAdd = result.nodes.filter((n) => n.entity && !existingEntities.has(n.entity));
    if (!toAdd.length) {
      this._nodeScanMessage = result.nodes.length
        ? "All discovered Deco units are already added."
        : "No TP-Link Deco satellite units found.";
      this.requestUpdate();
      return;
    }
  ...
  _scanIndividualDevices() {
    const result = scanTplinkDevices(this.hass);
    this._discoveredDevices = result.individualDevices;
    this._discoveredSelected = new Set();
    this.requestUpdate();
  }
  ...
  _addSelectedDiscovered() {
    ...
    const toAdd = (this._discoveredDevices || [])
      ...
      .map((d) => ({ ...DEFAULT_INDIVIDUAL_DEVICE, entity: d.entity }));
  ...
  _renderDiscoveredDevicesList() {
    const existing = new Set((this._config.individual_devices || []).map((d) => d.entity));
    const list = this._discoveredDevices || [];
    if (!list.length) {
        ...
        return html`
          <div class="list-item" style="opacity:${already ? 0.5 : 1}">
            <div class="list-item-info">
              ...
              <span>${d.name} <span style="color:var(--secondary-text-color); font-size:0.85em;">(${d.source})</span></span>
            </div>
            ${already
  ...
  _renderIndividualDevicesPage() {
    ...
    return html`
      <div class="form-section">
        <div class="sub-header">Auto-Discovery (TP-Link)</div>
        <p style="color:var(--secondary-text-color); font-size:0.9em; margin:0 0 8px 0;">
          Scans for device_tracker entities from the TP-Link Deco and
          TP-Link Router integrations (mesh units and the router itself
          are excluded - use the Nodes and Router pages for those).
          Nothing is added automatically; review the list and select
          which ones you want, then add them like any other device -
          icon, color, and group are all still yours to set afterward.
        </p>
        <button class="add-btn" @click=${() => this._scanIndividualDevices()}>
          Scan for TP-Link Devices
        </button>
