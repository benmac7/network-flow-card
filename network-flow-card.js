...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    // The trunk-drop (vertical line feeding DOWN from Router/Primary AP
    // into the bus-line) needs to align with wherever that circle above
    // it actually is - NOT with the midpoint of its own children below.
    // Those are two different reference points, and when the branches
    // below are asymmetric (e.g. a narrow primary-devices column next to
    // a full AP column), their midpoint doesn't coincide with where the
    // circle above is actually centered - that was the bug in 1.4.8.
    const trunkDrop = branches.querySelector(".trunk-drop");
    if (trunkDrop) {
      const trunkEl = root.querySelector(".trunk");
      // Scoped to only direct trunk-chain circles (Internet, Router,
      // Primary AP) - explicitly NOT LAN's circle, which is nested one
      // level deeper inside Router's own wrapper and would otherwise be
      // picked up as "last" (incorrectly) whenever LAN is configured but
      // no Primary AP exists.
      ...
      if (feederCircle) {
        ...
        const trunkMarginLeft = feederCenterX - branchesRect.left - 1;
