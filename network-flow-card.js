...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    // Reset any previous shift before measuring, so each pass starts
    // from a clean, un-shifted baseline instead of compounding on top
    // of a previous correction.
    branches.style.transform = "";
    const branchesRect = branches.getBoundingClientRect();

    // --- Bus-line's own reach: needs to touch the actual VISIBLE
    // content (a line strand, or a fillline), not the outer edge of the
    // column container around it. Using the wide column container here
    // (as 1.5.3 did) reintroduced the exact overshoot bug from 1.4.6,
    // since a column's own edge can coincide with the grid's edge.
    const lineCandidates = Array.from(
      branches.querySelectorAll(".vline, .vline-offline, .ap-col-fillline")
    );
    if (lineCandidates.length >= 2) {
      const lineRects = lineCandidates.map((el) => el.getBoundingClientRect());
      const leftMostLine = lineRects.reduce((a, b) => (a.left < b.left ? a : b));
      const rightMostLine = lineRects.reduce((a, b) =>
        a.left + a.width > b.left + b.width ? a : b
      );
      const leftMargin = Math.max(0, leftMostLine.left - branchesRect.left);
      const rightMargin = Math.max(0, branchesRect.right - (rightMostLine.left + rightMostLine.width));
      ...
      // Fall back to the CENTER (not edge) of the wide column containers -
      // less precise than touching the actual strokes, but a safer
      // fallback than their edges, which is what caused the overshoot.
      const wideCandidates = Array.from(
        branches.querySelectorAll(".ap-col-line, .ap-col-fillline-wrap")
      );
      if (wideCandidates.length >= 2) {
        const wideRects = wideCandidates.map((el) => el.getBoundingClientRect());
        const leftMostWide = wideRects.reduce((a, b) => (a.left < b.left ? a : b));
        const rightMostWide = wideRects.reduce((a, b) =>
          a.left + a.width > b.left + b.width ? a : b
        );
        const leftCenter = leftMostWide.left + leftMostWide.width / 2;
        const rightCenter = rightMostWide.left + rightMostWide.width;
        const leftMargin = Math.max(0, leftCenter - branchesRect.left);
        const rightMargin = Math.max(0, branchesRect.right - (rightMostWide.left + rightMostWide.width / 2));
        busLine.style.marginLeft = `${leftMargin}px`;
        busLine.style.marginRight = `${rightMargin}px`;
        this._busMarginLeft = leftMargin;
        this._busMarginRight = rightMargin;
    ...
    // --- Whole-block shift: this needs the TRUE overall extent of the
    // branches content (to compute its real center), so the wide row-3
    // containers (always rendered, explicit width:100%) are the right
    // tool here - confirmed working in 1.5.3.
    const shiftCandidates = Array.from(
      branches.querySelectorAll(".ap-col-line, .ap-col-fillline-wrap")
    );
    if (shiftCandidates.length >= 2) {
      ...
      if (feederCircle) {
        ...
        const branchesCenterX = branchesRect.left + branchesRect.width / 2;
        const shiftX = feederCenterX - branchesCenterX;
          ...
          <div class="flow-main-layout pos-${summaryPos}">
            ...
            <div class="flow-diagram">
              ${this._renderTrunk(
                ...
                config.primary_badge_size ?? 18,
                config.primary_badge_icon_size ?? 12,
                config.primary_badge_show_tiered ?? false
              )}
              ${branchApData.length
                ? this._renderBranches(
                    ...
                    config.primary_badge_size ?? 18,
                    config.primary_badge_icon_size ?? 12,
                    config.primary_badge_show_flat ?? true
                  )
                : null}
            </div>
  ...
  _renderTrunk(
    ...
    primaryBadgeOutlineColor,
    primaryBadgeIcon,
    primaryBadgeSize,
    primaryBadgeIconSize,
    primaryBadgeShowTiered
  ) {
              ...
              >
                ...
                ${primaryBadgeShowTiered
                  ? html`<div
                      ...
                      style="background:${primaryBadgeColor}; color:${primaryBadgeIconColor}; border:2px solid ${primaryBadgeOutlineColor}; width:${primaryBadgeSize ?? 18}px; height:${primaryBadgeSize ?? 18}px;"
                    >
                      <ha-icon icon="${primaryBadgeIcon || 'mdi:star'}" style="--mdc-icon-size:${primaryBadgeIconSize ?? 12}px"></ha-icon>
                    </div>`
                  : null}
              </div>
  ...
  _renderBranches(apData, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, primaryBadgeColor, primaryBadgeIconColor, primaryBadgeOutlineColor, primaryBadgeIcon, primaryBadgeSize, primaryBadgeIconSize, primaryBadgeShowFlat) {
    ...
    return html`
      ...
      >
        ${collapseTbar
          ...
          : routerOffline
          ? html`
              <div class="trunk-drop" style="display:flex; justify-content:center; align-items:center;">
                <div class="offline-x-mid offline-x-pulse" style="color:var(--error-color, #f44336)">
                  <ha-icon icon="mdi:close"></ha-icon>
                </div>
              </div>
  ...
  _renderAdvancedPage() {
    ...
    return html`
      <div class="form-section">
        ...
        <div class="sub-header">Primary AP Badge</div>
        <div class="toggle-row">
          <span>Show in Tiered Layout</span>
          <ha-switch
            .checked=${this._config.primary_badge_show_tiered ?? false}
            @change=${(e) => this._valueChanged(e, "primary_badge_show_tiered")}
          ></ha-switch>
        </div>
        <div class="toggle-row">
          <span>Show in Flat Layout</span>
          <ha-switch
            .checked=${this._config.primary_badge_show_flat ?? true}
            @change=${(e) => this._valueChanged(e, "primary_badge_show_flat")}
          ></ha-switch>
        </div>
        <ha-icon-picker
          .label=${"Badge Icon"}
