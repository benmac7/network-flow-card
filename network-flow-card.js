...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    // Align the feeder circle above to the MIDPOINT BETWEEN THE TWO
    // OUTERMOST COLUMNS' OWN CENTERS - not the whole block's geometric
    // centroid. Those only coincide when both columns are similar
    // widths; when one column (e.g. Primary's own devices connector)
    // shrinks much narrower than the other, the centroid gets pulled
    // toward the wider column while the true midpoint stays balanced.
    // Derived from the same leftMost/rightMost pair used above so the
