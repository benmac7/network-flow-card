...
class NetworkFlowCard extends LitElement {
  ...
  _alignBusLine() {
    ...
    // Measure the row-3 containers directly - .ap-col-line (each AP's
    // line area) and .ap-col-fillline-wrap (Primary's own column area).
    // These are ALWAYS rendered regardless of what's inside them (even
    // an "empty" line area still renders this container), and both have
    // an explicit width:100% set specifically so their box always spans
    // the true column width - making them a reliable measurement target
    // regardless of whether their actual content (a line, a fillline, or
    // nothing) is present. Measuring inner stroke elements directly
    // (.vline etc.) seemed more precise in principle, but proved
    // unreliable in practice - inspector data confirmed the correction
    // was computing exactly 0px in configurations where those inner
    // elements weren't reliably found.
    let candidates = Array.from(
      branches.querySelectorAll(".ap-col-line, .ap-col-fillline-wrap")
    );
    if (candidates.length < 2) {
      candidates = Array.from(
        branches.querySelectorAll(
          ".ap-col-circle, .ap-col-devcircle, .vline, .vline-offline, .ap-col-fillline"
        )
      );
    }
    ...
    // Rather than independently repositioning the trunk-drop to match
    // the circle above it (1.4.8/1.5.0 - this fixed the trunk-drop's own
    // alignment with "Test" above, but left the AP columns below
    // positioned by CSS Grid's own internal centering, which knows
    // nothing about where "Test" actually sits - so the two disagreed
    // whenever the branches were asymmetric), shift the ENTIRE branches
    // block as one unit so ITS natural center lines up with the circle
    // above. Everything inside (trunk-drop, bus-line, AP columns) keeps
    // its correct position relative to everything else; only the whole
    // block's position shifts to match.
