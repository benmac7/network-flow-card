...
class NetworkFlowCard extends i {
  ...
  _renderBranches(columns, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, minDur, maxDur, primaryOriginalIndex = -1, tierCount = 2) {
    ...
    const apLineInfo = (ap) => {
      ...
      return {
        hasBandwidth: !!(ap.entities.download || ap.entities.upload),
        dlDur,
        ulDur,
      ...
      return b`
        <div class="ap-col-line ${!hasAnySwitch ? "bus-top-connector" : ""}" style="grid-column:${cs}; grid-row:${row};">
          ${hideApOwnLine
            ...
            ? this._renderXOnlyVerticalStretch()
            : line.hasBandwidth
            ? this._verticalDualLine(busLineColor, busLineColor, line.dlDur, line.ulDur, animate, "100%", !line.wired)
            ...
            return b`
              <div class="ap-col-line bus-top-connector" style="grid-column:${topGridCol(col, i)}; grid-row:3;">
                ${hideApOwnLine || routerOffline
                  ? null
                  : topAp.hasBandwidth
                  ? this._verticalDualLine(busLineColor, busLineColor, topAp.dlDur, topAp.ulDur, animate, "100%", !topAp.wired)
...
class NetworkFlowCardEditor extends i {
          ...
          return b`
            <div class="tree-node">
              <div class="tree-card">
                ...
                ${this._renderAddRow(holderPath, isHomelabNode, !isApNode)}
              </div>
              ${expanded ? this._renderTreeChildren(holderPath, holder) : null}
            </div>
          `;
        })}
          ...
          return b`
            <div class="tree-node">
              <div class="tree-card">
                ...
                ${this._renderAddRow(path, false, false)}
              </div>
              ${expanded ? this._renderTreeChildren(path, ap) : null}
            </div>
          `;
        })}
        ${feds.map((sw, i) => {
          ...
          return b`
            <div class="list-item tree-item">
              <div class="list-item-info">
                ...
                <span class="tree-label">${this._treeLabel(hl, `Server ${i + 1}`)}</span>
              </div>
              <div class="list-item-actions">
  ...
  _renderChildLists(holderPath, holder, noun = "Switch") {
    ...
    const feds = holder.fed_switches || [];
    const homelabs = holder.homelabs || [];
    // An Access Point feeds switches and Homelabs; its own Access Point list
    // only shows up if the YAML already has one (a wireless mesh, say).
    const showAps = noun === "Switch" || aps.length > 0;
    return b`
      ${showAps ? b`
  ...
  _renderIndividualDevicesPage() {
    ...
    return b`
      <div class="form-section">
        ...
        ${this._infoHeader(
          "Auto-Discovery (TP-Link / OpenWrt / Synology SRM / AsusRouter / UniFi / Omada / Netgear)",
          "Scans for device_tracker entities from the TP-Link Deco, TP-Link Router, OpenWrt (LuCI), Synology SRM, AsusRouter, UniFi, TP-Link Omada, and Netgear integrations (mesh units, switches, access points, and routers/gateways are excluded - use the Nodes and Router pages for those). Netgear's own integration never tells a satellite/switch/AP apart from an ordinary client, so every one of its attached devices shows up here rather than on the Nodes page. Nothing is added automatically; review the list and select which ones you want, then add them like any other client - icon, color, name, and group are all still yours to set afterward."
        )}
