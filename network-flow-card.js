...
class NetworkFlowCard extends LitElement {
  ...
  _renderPrimaryApBadge(ap, stackIndex = 0) {
    ...
    const posStyle = badgeCornerStyle(cfg.primary_badge_location, stackIndex, size);
    return html`<div
  ...
  _renderPoeBadge(hass, switchEntityId, poeConfig, stackIndex = 0) {
    ...
    const flashClass =
      poeConfig.animate_over_threshold && isAbovePoeTopThreshold(total, poeConfig)
        ? "poe-badge-flash"
        : "";
    const posStyle = badgeCornerStyle(this._config.poe_badge_location, stackIndex, badgeSize);
    return html`<div
      class="poe-badge ${flashClass}"
      style="${posStyle} background:${bg}; color:${textColor}; height:${badgeSize}px; line-height:${badgeSize}px; padding:0 ${hPad}px; border-radius:${Math.round(badgeSize / 2)}px; font-size:${fontSize}px;"
    >
          ...
          ? html`
              ...
              >
                ...
                ${(() => {
                  ...
                  const stacks = computeBadgeStacks([
                    { key: "primary", location: this._config.primary_badge_location, visible: ap.show_primary_badge !== false },
                    { key: "poe", location: this._config.poe_badge_location, visible: computePoeTotal(hass, ap.entity, ap.poe) != null },
                    { key: "vpn", location: this._config.vpn_badge_location, visible: this._config.vpn_target === "primary_ap" && !!this._config.vpn_entity },
                    { key: "firewall", location: this._config.firewall_badge_location, visible: this._config.firewall_target === "primary_ap" && !!this._config.firewall_entity }
                  ]);
                  return html`
                    ${this._renderPrimaryApBadge(ap, stacks.primary || 0)}
                    ${this._config.vpn_target === "primary_ap" ? this._renderVpnBadge(hass, this._config.vpn_entity, stacks.vpn || 0) : null}
                    ${this._config.firewall_target === "primary_ap" ? this._renderFirewallBadge(hass, this._config.firewall_entity, stacks.firewall || 0) : null}
  ...
  _renderBranches(columns, animate, busLineColor, hasIndividualDevices, individualDevicesLineColor, hass, routerOffline, routerOfflineColor, routerEnabled, primaryApItem, primaryApLayout, minDur, maxDur, primaryOriginalIndex = -1) {
    ...
    const columnHasDevices = (c) => c.subSwitch ? !!c.subSwitch.entities.connected_devices : c.ap ? !!c.ap.entities.connected_devices : !!(c.switch && c.switch.entities.connected_devices);
          ...
          if (col.subSwitch) {
            ...
            if (col.switchGroupSize > 1 && col.switchGroupStart) {
              ...
              return html`
                ...
                <div class="ap-col-line ap-col-line-grouped" style="grid-column:${apCol(i)}; grid-row:5;">
                  ${subOffline ? this._renderXOnlyVerticalStretch() : this._verticalPassThroughLine(busLineColor, columnDurations[i], false)}
                </div>
              `;
            }
            if (col.switchGroupSize > 1 && !col.switchGroupStart) {
            ...
            return html`
              <div class="ap-col-line" style="grid-column:${apCol(i)}; grid-row:5;">
                ${subOffline ? this._renderXOnlyVerticalStretch() : this._verticalSingleLine(busLineColor, 3, false, "100%", false)}
              </div>
            `;
          }
          ...
          return html`
            ...
            >
              ...
              ${(() => {
                ...
                const stacks = computeBadgeStacks([
                  { key: "primary", location: this._config.primary_badge_location, visible: ap.show_primary_badge !== false },
                  { key: "poe", location: this._config.poe_badge_location, visible: poeVisible },
                  { key: "vpn", location: this._config.vpn_badge_location, visible: this._config.vpn_target === "primary_ap" && !!this._config.vpn_entity },
                  { key: "firewall", location: this._config.firewall_badge_location, visible: this._config.firewall_target === "primary_ap" && !!this._config.firewall_entity }
                ]);
                return html`
  ...
  static get styles() {
    return css`
      ...
      /* Icon-bearing badges (guest, primary AP, VPN, firewall) need the
         same explicit centering rule applied directly to the ha-icon
         ...
         PoE has no icon of its own here (it's a text badge), so it's
         excluded. */
      .guest-badge ha-icon,
  ...
  _renderAdvancedPage() {
    ...
    return html`
      <div class="form-section">
        ...
        ${this._renderLocationSelect(this._config.primary_badge_location, "primary_badge_location")}
        ...
        ${this._renderLocationSelect(this._config.poe_badge_location, "poe_badge_location")}
      </div>
    `;
  }
