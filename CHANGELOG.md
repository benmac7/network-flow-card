...
This version is built directly on top of v1.5.5 (not v1.5.6, which had
an unresolved loading issue in some dashboard configurations - if
you're upgrading from v1.5.6, this release intentionally does not
carry forward its bus-line/branches alignment changes).
...
- **Primary AP badge visibility toggles**: separate on/off switches for
  showing the badge in Tiered layout and in Flat layout (Advanced →
  Primary AP Badge). Previously the badge only ever appeared in Flat
  layout with no way to show it in Tiered, or hide it in Flat.
