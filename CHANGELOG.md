...
### Added
- **Primary AP badge size and icon size** are now configurable
  (Advanced → Primary AP Badge → Badge Size / Badge Icon Size).

### Fixed
- Badge positioning now scales proportionally with the AP's circle size
  instead of using a fixed pixel offset - previously, enlarging an AP's
  circle would visibly push the badge outside/away from the circle's
  edge, since the offset didn't account for the circle's own size.

## [1.6.0]

### Added
- **Reorder Individual Devices** — move up/down buttons in the
  Individual Devices list, matching the same reordering already
  available for Access Points.

## [1.5.6]

### Fixed
- **Actual root cause of the "shifts left" bug, confirmed mathematically
  rather than guessed.** The whole-block shift (introduced in 1.5.1) was
  aligning the feeder circle above to `.branches`' own geometric
  centroid - correct only when both branch columns are similar widths.
  When Primary AP's own column shrinks to just a thin connecting line
  (no Connected Devices circle) next to a full-width AP column, the
  centroid gets pulled hard toward the wider column, while the true
  midpoint between the two branches' own centers stays roughly balanced.
  Using the centroid was therefore skewing the whole block toward
  whichever column happened to be wider. Now aligns to the midpoint
  between the two outermost columns' own centers instead, and this is
  derived from the exact same measurement pass used for the bus-line's
  own edges, so the two can no longer disagree with each other even in
  fallback scenarios.

## [1.5.5]

### Fixed
- **Card content could visually paint over other dashboard elements**
  (e.g. a custom fixed bottom-nav/dock card) in some dashboard setups.
  `ha-card` didn't establish its own CSS stacking context, so the card's
  internal z-index values (all modest - 1 through 3, used only to layer
  circles/badges above lines within the card itself) had a technical
  path to compete with unrelated sibling elements elsewhere on the page,
  depending on how those elements were implemented. Added
  `isolation: isolate` to `ha-card`, which guarantees all of the card's
  internal stacking is fully contained within it - it can no longer
  affect, or be impacted by, anything else on the dashboard.

## [1.5.4]

### Fixed
- **T-bar overshoot, reintroduced by 1.5.3's own fix.** 1.5.3 fixed AP
  positioning by measuring the wide, always-present row-3 containers -
  but then used those same wide measurements for the bus-line's own
  reach too, which is a different job with different requirements. The
  bus-line needs to touch the actual visible line content; the wide
  containers' edges coincide with the grid's own edges (the same root
  cause diagnosed back in 1.4.6), so reusing them for both purposes
  reintroduced that exact overshoot. Split into two separate,
  purpose-specific measurements: the actual visible strokes (`.vline`,
  `.vline-offline`, `.ap-col-fillline`) for the bus-line's reach - this
  was already confirmed correct in 1.4.7 - and the wide row-3 containers
  only for computing the whole block's true center, which is what 1.5.3
  actually fixed and should remain unaffected by this change.

## [1.5.3]

### Fixed
- **Confirmed via inspector data**: `.branches` was correctly receiving
  `transform: translateX(0px)` - meaning the 1.5.1/1.5.2 alignment code
  was running, but computing exactly zero because it couldn't reliably
  find 2+ valid candidates via the inner-stroke-element selector
  (`.vline`, `.vline-offline`, `.ap-col-fillline`) in the "Primary AP has
  no Connected Devices" configuration, and the 1.5.2 fallback selector
  also came up short. Switched to measuring `.ap-col-line` and
  `.ap-col-fillline-wrap` directly instead - these are the row-3
  containers that are always rendered regardless of what's inside them,
  and both already carry an explicit `width: 100%` (added in an earlier
  fix) specifically so their box always spans the true column width.
  Verified both elements are actually present via the reporter's own
  inspector output before shipping this.

## [1.5.2]

### Changed
- Added a fallback candidate selector for the branches-alignment
  measurement: if the strict stroke-only selector (`.vline`,
  `.vline-offline`, `.ap-col-fillline`) finds fewer than two elements in
  some configuration, it now falls back to broader container elements
  rather than silently skipping the correction and reverting to CSS's
  own (previously shown to be unreliable) centering. This is a
  defensive fix based on the most likely explanation for "works when
  Primary AP has Connected Devices, regresses when it doesn't" - I
  verified `.ap-col-fillline` is present in both code paths, so I'm not
  fully certain this is the actual root cause rather than a related
  symptom. If this doesn't resolve it, the next step is inspecting
  `.branches`' computed `transform` value directly in both
  configurations to see whether a correction is being applied at all.

## [1.5.1]

### Fixed
- **AP still visibly off-center relative to the trunk-drop (Tiered
  layout, asymmetric branches).** The 1.5.0 fix correctly repositioned
  the trunk-drop to match the Primary AP's circle above it, but left the
  AP columns below positioned by CSS Grid's own internal centering -
  which has no awareness of where that circle actually sits. The two
  could still disagree whenever the branches were asymmetric (e.g. one
  real AP next to a narrow devices-only column), which is exactly what
  was seen: the trunk-drop lined up correctly, but the AP still looked
  pulled toward it. Replaced the trunk-drop-specific fix with a
  whole-block shift instead: the entire branches grid (trunk-drop,
  bus-line, and every AP column together) now shifts as a single unit so
  its natural center aligns with the circle above, preserving every
  internal position relative to the others and only adjusting the
  block's overall placement.

## [1.5.0]

### Fixed
- **Trunk-drop misaligned in Tiered layout (regression from 1.4.8).** The
  1.4.8 fix aligned the trunk-drop to the midpoint of its own children in
  the branches grid below it - but the trunk-drop's actual job is to
  connect down from the circle *above* it (Router or the Primary AP), a
  completely different reference point. When the branches below were
  asymmetric, those two points diverged, visibly skewing the line and
  everything below it. Now aligns directly with the actual circle
  feeding into it from above, scoped carefully to exclude LAN's circle
  (which sits nested inside Router's own wrapper and would otherwise be
  incorrectly picked up as "last" when LAN is configured without a
  Primary AP).
- Left/right-positioned summary badges now have a bit more breathing
  room from the card title above them, instead of sitting flush against
  it.

### Changed
- Download and Upload summary badges are now hidden entirely when their
  respective entity isn't configured, instead of showing a badge with a
  "-" placeholder. Each is controlled independently.

## [1.4.9]

### Added
- **Primary AP badge icon** is now customizable (Advanced → Primary AP
  Badge → Badge Icon), alongside the existing background/icon/outline
  color options. Defaults to `mdi:star`.

## [1.4.8]

### Fixed
- **Trunk-drop skewed off-center with two unevenly-sized branches
  (Tiered layout).** With exactly one secondary AP, the layout has two
  columns: the Primary AP's own devices/Individual-Devices connector
  (often narrow) and the secondary AP itself (a full-width column). The
  trunk-drop was using CSS `justify-self: center`, which centers against
  the *whole track group's* bounding box - with two very unevenly sized
  columns, that geometric center doesn't land at the true midpoint
  between the two branches, making the wider one look pulled toward
  center. It's now repositioned via the same measurement pass used for
  the bus-line, to the actual midpoint between the two outermost
  branches.
- **Gap in the dotted line from Primary AP to Individual Devices**, when
  the Primary AP has no Connected Devices entity of its own but a
  secondary AP does. The connecting line for Primary's column stopped
  one row short of where it needed to, leaving an unfilled gap the exact
  height of the secondary AP's devices circle in the row next to it.
