...
### Added
- **Switch-with-APs Nodes (Phase 3)**: a Node can now be a Switch
  feeding one or more of its own Access Points. Add APs to a Switch
  node from its editor page ("+ Add Access Point" under Access Points)
  - each gets its own full AP editor (bandwidth, backhaul, connected
    devices, colors), identical to a standalone AP.
  - The Switch renders once, centered above its group of APs, with its
    own connecting bus-line feeding down into each AP - all lines
    within a group render solid, since these represent the switch's own
    wired ports rather than wireless backhaul.
  - An AP inside a Switch group can still be flagged Primary (for the
    star badge and VPN/Firewall targeting), but per the established
    rule, Tiered layout is not available for it - the card silently
    renders as Flat in this case regardless of the global layout
    setting.
- This is a first working version of the nested structure; alignment
  polish (particularly for uneven group sizes) may need refinement in
  a follow-up.
