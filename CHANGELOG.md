...
- **Client sub-group boxes all stretched to match whichever one was
  tallest**, making every group huge the moment any single group
  wrapped onto multiple rows - a direct interaction between
  `align-items: stretch` on their shared flex container (present all
  along, but previously close to invisible since every group's natural
  height was similar) and the 32px row-gap added in 3.1.6 for wrapped
  rows, which could make one heavily-populated group's natural height
  balloon and then get imposed on every sibling regardless of its own
  content. Changed to `align-items: flex-start` so each box now sizes
  to only its own content, independent of its siblings.
