...
### Changed
- **Summary is now fully user-selectable**: replaced the separate VPN
  "Show in Summary" / Firewall "Show in Summary" toggles with three
  "Summary Item" dropdowns in Advanced → Layout. Choose any combination
  (in any order) of Download, Upload, Latency and Jitter, VPN Status, or
  Firewall Status - up to 3 shown at once. Defaults to Download / Upload
  / Latency and Jitter, matching previous behavior. Each item still only
  appears if its underlying entity is actually configured.

## [1.5.0]

### Added
- **Firewall badge** - a new badge mirroring the VPN badge exactly:
  single entity, "Applies To" target (Router or Primary AP), Animate
  when Offline, Show in Summary, and full active/offline color and icon
  customization. Defaults to `mdi:wall-fire` with an orange background.
  Positioned at the bottom-right of the circle (VPN stays top-right), so
  both can be shown simultaneously without overlapping.
