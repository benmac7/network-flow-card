...
### Changed
- VPN offline flash animation slowed slightly (1.2s → 1.8s per cycle).
- VPN summary status text changed from "Connected"/"Disconnected" to
  "Online"/"Offline".
- **Color picker layout**: every color selector across all editor pages
  now shows its label on its own row, with the color swatch and hex
  input on the row below - instead of squeezing label and selector onto
  the same row. This was especially cramped inside two-column layouts;
  the fix applies everywhere for consistency.
