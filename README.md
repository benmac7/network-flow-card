...
- **Router/Gateway node** — fully optional. Leave its entity blank and it
  (and the LAN node hanging off it) disappear automatically.
...
- **Nodes** — the bus below Router/Switch is made up of Nodes, and each one
  ...
  - A **Switch feeding one or more Access Points and/or Switches** — each
    fed item gets its own full configuration (an AP keeps its normal
    bandwidth/backhaul options; a fed Switch is a leaf with its own
    Connected Devices and PoE), connected via a small secondary bus-line
    splitting off the parent Switch, with solid lines throughout for any
    fed Switch.
...
- **PoE monitoring on the Router, Switches, and Access Points** — a summed
  wattage badge on any of them, sourced automatically from every
  PoE-related entity on that device, or from a manually chosen list (works
  even without a status entity configured). Color it with a single fixed
  color or up to three configurable thresholds (e.g. green/orange/red),
  with an optional flash animation once the total exceeds the top
  threshold. Badge size and font size are independently adjustable. A
  matching **PoE Total** Summary item adds up every PoE badge shown across
  the whole diagram. Defaults to Off until you configure it.
- **Configurable badge positions** — Primary AP, VPN, Firewall, and PoE
  badges can each be placed in any of the four corners of their circle.
  PoE location is set per device (inside its own PoE section) and Primary
  AP badge location is set per Access Point (under its "Show Primary AP
  Badge" toggle), since different devices often want their badge
  somewhere different; VPN and Firewall location remain global settings.
  Badges that land in the same corner fan out slightly so both stay
  visible instead of one hiding the other.
- **Unique default colors per device type** — Internet, Router, LAN,
  Switch, Access Point, and Individual Device each default to their own
  distinct theme color out of the box, fully overridable per item as
  always.
...
**Internet**, then **Router/Gateway** (optional), **Switch** (optional), and
